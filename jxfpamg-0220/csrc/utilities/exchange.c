...
/*!
 * \fn JXF_Int jxf_DataExchangeList
...
{  
   ...
   JXF_Int  ierr = 0;
   JXF_Int  i;
   JXF_Int  terminate, responses_complete;
   JXF_Int  children_complete;
   JXF_Int  contact_flag;
   JXF_Int  proc;
   JXF_Int  contact_size;

   JXF_Int  size, post_size, copy_size;
   JXF_Int  total_size, count;      

   void *start_ptr = NULL, *index_ptr=NULL;
   JXF_Int  *int_ptr=NULL;  
   ...
   JXF_Int  recv_contact_buf_size = 0;
   JXF_Int  response_message_size = 0;

   JXF_Int  overhead;
 
   JXF_Int  max_response_size_bytes;
   JXF_Int  max_response_total_bytes;
   
   void **post_array = NULL;  /* this must be set to null or realloc will crash */
   JXF_Int  post_array_storage = 0;
   JXF_Int  post_array_size = 0;
   JXF_Int  num_post_recvs =0;
   ...
   const JXF_Int contact_tag = 1000*rnum;
   const JXF_Int response_tag = 1002*rnum;
   const JXF_Int term_tag =  1004*rnum;  
   const JXF_Int post_tag = 1006*rnum;
   ...
   /* if the response_obj_size or contact_obj_size is 0, set to sizeof(JXF_Int) */
   if (!response_obj_size) response_obj_size = sizeof(JXF_Int);
   if (!contact_obj_size) contact_obj_size = sizeof(JXF_Int);

   max_response_size_bytes = max_response_size*response_obj_size;
                
   /* pre-allocate the max space for responding to contacts */
   overhead = ceil((JXF_Real) sizeof(JXF_Int)/response_obj_size); /* for appending an integer */
   ...
   send_response_buf = jxf_MAlloc(max_response_total_bytes);
      
   /* allocate space for inital recv array for the responses - give each processor size max_response_size */

   initial_recv_buf = jxf_MAlloc(max_response_total_bytes*num_contacts);
   response_recv_buf_starts = jxf_CTAlloc(JXF_Int, num_contacts+1);  

   contact_ptrs = jxf_TAlloc( void *, num_contacts);
   response_ptrs = jxf_TAlloc(void *, num_contacts);
   ...
   if (num_contacts > 0 )
   {
      ...
      response_requests = jxf_CTAlloc(MPI_Request, num_contacts);
      response_statuses = jxf_CTAlloc(MPI_Status, num_contacts);
      contact_requests = jxf_CTAlloc(MPI_Request, num_contacts);
      contact_statuses = jxf_CTAlloc(MPI_Status, num_contacts);
      ...
      for (i = 0; i < num_contacts; i ++) 
      {
         ...
         jxf_MPI_Irecv(response_ptrs[i], max_response_total_bytes, MPI_BYTE, contact_proc_list[i], 
      ...
      for (i = 0; i < num_contacts; i ++) 
      {
         ...
         jxf_MPI_Isend(contact_ptrs[i], size*contact_obj_size, MPI_BYTE, contact_proc_list[i], 
   ...
   if (num_procs > 1) 
   { 
      ierr = jxf_CreateBinaryTree(myid, num_procs, &tree);
      ...
      term_requests = jxf_CTAlloc(MPI_Request, tree.num_child); 
      term_statuses = jxf_CTAlloc(MPI_Status, tree.num_child);

      for (i = 0; i < tree.num_child; i ++) 
      {
	 jxf_MPI_Irecv(NULL, 0, JXF_MPI_INT, tree.child_id[i], term_tag, comm, 
		   &term_requests[i]);
      }
   ...
   while (!terminate)
   {
      ...
      jxf_MPI_Iprobe(MPI_ANY_SOURCE, contact_tag, comm, &contact_flag, &status);

      while (contact_flag)
      {
         /* received contacts - from who and what do we do ?*/
         proc = status.MPI_SOURCE;
         jxf_MPI_Get_count(&status, MPI_BYTE, &contact_size);
         ...
         {
            recv_contact_buf = jxf_ReAlloc(recv_contact_buf, contact_obj_size*contact_size);
            recv_contact_buf_size = contact_size;
         } 


         /* this must be blocking - can't fill recv without the buffer */         
         jxf_MPI_Recv(recv_contact_buf, contact_size*contact_obj_size,  
        ...
        if (post_size > 0) /* we will need to send the extra information later */   
        {
           if (post_array_size == post_array_storage)
           {
              ...
              post_array = jxf_TReAlloc(post_array, void *, post_array_storage); 
              post_send_requests = jxf_TReAlloc(post_send_requests, MPI_Request, post_array_storage);
           }
           ...
           post_array[post_array_size] = jxf_MAlloc(size); 
           ...
           jxf_MPI_Isend(post_array[post_array_size], size, MPI_BYTE, proc, post_tag, 
        ...
        jxf_MPI_Send(send_response_buf, max_response_total_bytes, 
	...
	/* look for any more contact messages */
      ...
      {
	ierr = jxf_MPI_Testall(num_contacts, response_requests, &responses_complete, 
                           response_statuses);
        if (responses_complete && num_procs == 1) terminate = 1; /*added 11/08 */
                                     ...
                                     response messages? */
      {
         ierr = jxf_MPI_Testall(tree.num_child, term_requests, &children_complete, 
			    term_statuses);
	 ...
	 if (children_complete & (myid > 0)) /* root does not have a parent */ 
         {
           jxf_MPI_Isend(NULL, 0, JXF_MPI_INT, tree.parent_id, term_tag, comm, &request_parent);  
           jxf_MPI_Irecv(NULL, 0, JXF_MPI_INT, tree.parent_id, term_tag, comm, &term_request1);
         }
      }
	...
	if (myid == 0) /* root doesn't have a parent */
	   ...
	   terminate = 1;
        ...
        {
	   jxf_MPI_Test(&term_request1, &terminate, &term_status1);
        }  
        if(terminate) /*tell children to terminate */
        {
            if (myid > 0 ) jxf_MPI_Wait(&request_parent, &status_parent);
            
	    for (i = 0; i < tree.num_child; i ++)
	    {  /*a blocking send  - recv has been posted already */
	       jxf_MPI_Send(NULL, 0, JXF_MPI_INT, tree.child_id[i], term_tag, comm);
   ...
   {
      jxf_TFree(recv_contact_buf);
   }
  
   jxf_Free(send_response_buf);
   jxf_TFree(contact_ptrs);
   jxf_TFree(response_ptrs);
   ...
   for (i = 0; i < num_contacts; i ++)
   {
      int_ptr = (JXF_Int *) ( (char *) start_ptr + max_response_size_bytes); /* the overhead JXF_Int */
   ...
   post_recv_requests = jxf_TAlloc(MPI_Request, num_post_recvs);
   post_recv_statuses = jxf_TAlloc(MPI_Status, num_post_recvs);
   post_ptrs = jxf_TAlloc(void *, num_post_recvs);

   /* second loop to post any recvs and set up recv_response_buf */
   response_recv_buf = jxf_MAlloc(total_size*response_obj_size);
   ...
   for (i = 0; i < num_contacts; i ++)
   {
      ...
      copy_size = jxf_min(response_message_size, max_response_size);
      ...
      if (max_response_size < response_message_size)
      {
         ...
         jxf_MPI_Irecv(post_ptrs[count], size, MPI_BYTE, contact_proc_list[i]     ,
   ...
   post_send_statuses = jxf_TAlloc(MPI_Status, post_array_size);


   /*----------------CLEAN UP------------------- */
  
   jxf_Free(initial_recv_buf);
      

   if (num_contacts > 0 ) 
   {
      /* these should be done */
      ierr = jxf_MPI_Waitall(num_contacts, contact_requests, 
                         contact_statuses);

      jxf_TFree(response_requests);
      jxf_TFree(response_statuses);
      jxf_TFree(contact_requests);
      jxf_TFree(contact_statuses);
   }
   ...
   if (num_post_recvs)
   {
      ierr = jxf_MPI_Waitall(num_post_recvs, post_recv_requests, 
                         post_recv_statuses);  
      jxf_TFree(post_recv_requests);
      jxf_TFree(post_recv_statuses);
      jxf_TFree(post_ptrs);
   }
   ...
   if (post_array_size)
   {
      ierr = jxf_MPI_Waitall(post_array_size, post_send_requests, post_send_statuses);

      jxf_TFree(post_send_requests);
      jxf_TFree(post_send_statuses);
 
      for (i = 0; i < post_array_size; i ++)
      {
         jxf_Free(post_array[i]);
      }
      jxf_TFree(post_array);
   }
   

   if (num_procs > 1)
   {
      jxf_TFree(term_requests);
      jxf_TFree(term_statuses);
 
      ierr = jxf_DestroyBinaryTree(&tree);
   }
...
/*!
 * \fn JXF_Int jxf_CreateBinaryTree
 * \brief Get the processors position in the binary tree 
 *        (i.e., its children and parent processor ids)
 * \date 2011/09/01  
 */ 
JXF_Int 
jxf_CreateBinaryTree( JXF_Int myid, JXF_Int num_procs, jxf_BinaryTree *tree )
{
   JXF_Int  i, proc, size = 0;
   JXF_Int  ierr = 0;
   JXF_Int  *tmp_child_id;
   JXF_Int  num = 0, parent = 0;
   ...
   /* allocate space */
   tmp_child_id = jxf_TAlloc(JXF_Int, size);
   ...
   jxf_BinaryTreeParentId(tree) = parent;
   jxf_BinaryTreeNumChild(tree) = num;
   jxf_BinaryTreeChildIds(tree) = tmp_child_id;

   return(ierr);
}

/*!
 * \fn JXF_Int jxf_DestroyBinaryTree()
 * \brief Destroy storage created by createBinaryTree.
 * \date 2011/09/01  
 */ 
JXF_Int 
jxf_DestroyBinaryTree(jxf_BinaryTree *tree) 
{

  JXF_Int ierr = 0;

  jxf_TFree(jxf_BinaryTreeChildIds(tree));
