...
/*!
 *  csr_ilu.c -- Symbolic factorization, numerical factoration, and forward/backward sweeps for CSR matrix
 *  Date: 2025/10/08
 */ 
...
static void jxf_qsplit(JXF_Real* a, JXF_Int* ind, JXF_Int n, JXF_Int ncut);
static void jxf_check_col_index(JXF_Int row, JXF_Int num, JXF_Int* q);
static void jxf_sortrow(JXF_Int num, JXF_Int* q);

/**
 * \fn JXF_Int jxf_SymbFactor(JXF_Int n, JXF_Int* rwptr, JXF_Int* colind, JXF_Int levfill,
 *                                           JXF_Int nzmax, JXF_Int* nzlu,
 *                                           JXF_Int* ijlu, JXF_Int* uptr)
 *
 * \brief Symbolic factorization of a CSR matrix A in compressed sparse row format,
 *		    with resulting factors stored in nzlu.
 *
 ...
 * \param ierr  integer pointer. Return error message with the following meaning (return).
 *                0  --> successful return.
...
JXF_Int jxf_SymbFactor(JXF_Int n, JXF_Int* rwptr, JXF_Int* colind, JXF_Int levfill, JXF_Int nzmax, JXF_Int* nzlu, JXF_Int* ijlu,
                             JXF_Int* uptr)
{
#if DEBUG_MODE > 0
    ...
    JXF_Int  icolindj, ijlum, i, j, k, m, ibegin, iend, Ujbeg, Ujend, NE;
    JXF_Int  head, prev, lm, actlev, lowct, k1, k2, levp1, lmk, nzi, rowct;
    JXF_Int  cinindex = 0;
    JXF_Int *rowll, *lastcol, *levels;
    JXF_Int  ierr;

    rowll   = (JXF_Int*)calloc(n, sizeof(JXF_Int));
    lastcol = (JXF_Int*)calloc(n, sizeof(JXF_Int));
    levels  = (JXF_Int*)calloc(nzmax, sizeof(JXF_Int));
    ...
    *nzlu = n + 1;
    ...
    for (i = 1; i <= n; ++i) { // 100
        ...
        if (nzi > 1) {
            ...
            //  ---------------------------------------------------------
            jxf_sortrow(nzi - 1, &rowll[1]);

            // check col index
            jxf_check_col_index(i, nzi - 1, &rowll[1]);
            //  ---------------------------------------------------------
                ...
                if (levfill != 0) {
                    ...
                    for (m = Ujbeg; m <= Ujend; ++m) { // 60
                        ...
                        if (lastcol[ijlum] != i) {
                            ...
                            levels[ijlum] = jxf_min(levels[ijlum], actlev);
                        }
...
/**
 * \fn void jxf_iluk (JXF_Int n, JXF_Real *a,JXF_Int *ja, JXF_Int *ia, JXF_Int lfil,
 *                     JXF_Real *alu, JXF_Int *jlu, JXF_Int iwk, JXF_Int *ierr, JXF_Int *nzlu)
 *
 ...
 * \date   11/03/2024
 */
void jxf_iluk(JXF_Int n, JXF_Real* a, JXF_Int* ja, JXF_Int* ia, JXF_Int lfil, JXF_Real* alu, JXF_Int* jlu, JXF_Int iwk,
                 JXF_Int* ierr, JXF_Int* nzlu)
{
#if DEBUG_MODE > 0
    printf("### DEBUG: [-Begin-] %s ...\n", __FUNCTION__);
#endif
    ...
    // locals
    JXF_Int   ju0, k, j1, j2, j, ii, i, lenl, lenu, jj, jrow, jpos, n2, jlev, NE;
    JXF_Real  t, s, fact;
    JXF_Int   cinindex = 0;
    JXF_Real* w;
    JXF_Int * ju, *jw, *levs;
    ...
    w    = (JXF_Real*)calloc(n, sizeof(JXF_Real));
    ju   = (JXF_Int*)calloc(n, sizeof(JXF_Int));
    jw   = (JXF_Int*)calloc(3 * n, sizeof(JXF_Int));
    levs = (JXF_Int*)calloc(iwk, sizeof(JXF_Int));
        ...
        for (k = ju[jrow]; k <= jlu[jrow + 1] - 1; ++k) { // 203
            ...
            if (j >= ii) {
                ...
                if (jpos == 0) {
                    ...
                    w[jpos]       = w[jpos] - s;
                    jw[n2 + jpos] = jxf_min(jw[n2 + jpos], jlev + levs[k] + 1);
                }
            } else {
...
/**
 * \fn void jxf_ilut (JXF_Int n, JXF_Real *a, JXF_Int *ja, JXF_Int *ia, JXF_Int lfil, JXF_Real droptol,
 *                     JXF_Real *alu, JXF_Int *jlu, JXF_Int iwk, JXF_Int *ierr, JXF_Int *nz)
 *
 ...
 * \date   11/03/2024
 */
void jxf_ilut(JXF_Int n, JXF_Real* a, JXF_Int* ja, JXF_Int* ia, JXF_Int lfil, JXF_Real droptol, JXF_Real* alu, JXF_Int* jlu,
                 JXF_Int iwk, JXF_Int* ierr, JXF_Int* nz)
{
#if DEBUG_MODE > 0
    printf("### DEBUG: [-Begin-] %s ...\n", __FUNCTION__);
#endif
    ...
    // locals
    JXF_Int   ju0, k, j1, j2, j, ii, i, lenl, lenu, jj, jrow, jpos, NE, len;
    JXF_Real  t, s, fact, tmp;
    JXF_Int   cinindex = 0;
    JXF_Real *w, *tnorm;
    JXF_Int * ju, *jw;
    ...
    ju    = (JXF_Int*)calloc(n, sizeof(JXF_Int));
    jw    = (JXF_Int*)calloc(2 * n, sizeof(JXF_Int));
    w     = (JXF_Real*)calloc(n + 1, sizeof(JXF_Real));
    tnorm = (JXF_Real*)calloc(n, sizeof(JXF_Real));
    ...
    for (ii = 1; ii <= n; ++ii) {
        ...
        j2  = ia[ii + 1] - 1;
        tmp = 0.0;
        for (k = j1; k <= j2; ++k) tmp = tmp + jxf_abs(a[k]);
        tmp       = tmp / (JXF_Real)(j2 - j1 + 1);
    ...
    F150:
        ...
        if (jxf_abs(fact) <= droptol) goto F150;
    ...
    F160:
        ...
        // update L-matrix
        lenl = len;
        len  = jxf_min(lenl, lfil);

        // sort by quick-split
        jxf_qsplit(&w[1], &jw[1], lenl, len);
        ...
        len = 0;
        for (k = 1; k <= lenu - 1; ++k) {
            //		if ( jxf_abs(w[ii + k])  >  droptol*tnorm )
            if (jxf_abs(w[ii + k]) > tnorm[ii]) {
                ++len;
                w[ii + len]  = w[ii + k];
                jw[ii + len] = jw[ii + k];
            }
        ...
        lenu = len + 1;
        len  = jxf_min(lenu, lfil);

        jxf_qsplit(&w[ii + 1], &jw[ii + 1], lenu - 1, len);
...
/**
 * \fn void jxf_ilutp (JXF_Int n, JXF_Real *a, JXF_Int *ja, JXF_Int *ia, JXF_Int lfil, JXF_Real droptol,
 *                      JXF_Real permtol, JXF_Int mbloc, JXF_Real *alu, JXF_Int *jlu, JXF_Int *iperm,
 *                      JXF_Int iwk, JXF_Int *ierr, JXF_Int *nz)
 *
 ...
 * \date   11/03/2024
 */
void jxf_ilutp(JXF_Int n, JXF_Real* a, JXF_Int* ja, JXF_Int* ia, JXF_Int lfil, JXF_Real droptol, JXF_Real permtol, JXF_Int mbloc,
                  JXF_Real* alu, JXF_Int* jlu, JXF_Int* iperm, JXF_Int iwk, JXF_Int* ierr, JXF_Int* nz)
{
#if DEBUG_MODE > 0
    printf("### DEBUG: [-Begin-] %s ...\n", __FUNCTION__);
#endif
    ...
    // local variables
    JXF_Int   k, i, j, jrow, ju0, ii, j1, j2, jpos, len, imax, lenu, lenl, jj, icut, NE;
    JXF_Real  s, tmp, tnorm, xmax, xmax0, fact, t;
    JXF_Int   cinindex = 0;
    JXF_Real* w;
    JXF_Int * ju, *jw;
    ...
    for (ii = 1; ii <= n; ++ii) { // 500
        ...
        tnorm = 0.0;
        for (k = j1; k <= j2; ++k) tnorm = tnorm + jxf_abs(a[k]); // 501
        if (tnorm == 0.0) goto F999;
        tnorm = tnorm / (JXF_Real)(j2 - j1 + 1);
...
// Symbolic factorization of a CSR matrix A
// JXF_Int jxf_CSRMatrixSymbFactor(jxf_CSRMatrix* A, JXF_Int levfill, JXF_Int nzmax, JXF_Int* nzlu, JXF_Int* ijlu, JXF_Int* uptr)
// {
//     JXF_Int  row = jxf_CSRMatrixNumRows(A);
...
JXF_Int jxf_CSRMatrixILUK(jxf_CSRMatrix* A, JXF_Int levfill, JXF_Real* alu, JXF_Int* jlu, JXF_Int iwk, JXF_Int* nzlu)
{
    JXF_Int      ierr   = JXF_SUCCESS;
    JXF_Int      row    = jxf_CSRMatrixNumRows(A);
    JXF_Int*     A_i    = jxf_CSRMatrixI(A);
    JXF_Int*     A_j    = jxf_CSRMatrixJ(A);
    JXF_Complex* A_data = jxf_CSRMatrixData(A);

    jxf_iluk(row, A_data, A_j, A_i, levfill, alu, jlu, iwk, &ierr, nzlu);

    return ierr;
}

JXF_Int jxf_CSRMatrixILUT(jxf_CSRMatrix* A, JXF_Int levfill, JXF_Real droptol, JXF_Real* alu, JXF_Int* jlu, JXF_Int iwk,
                                JXF_Int* nzlu)
{
    JXF_Int      ierr   = JXF_SUCCESS;
    JXF_Int      row    = jxf_CSRMatrixNumRows(A);
    ...
    jxf_ilut(row, A_data, A_j, A_i, levfill, droptol, alu, jlu, iwk, &ierr, nzlu);

    return ierr;
}

JXF_Int jxf_CSRMatrixILUTP(jxf_CSRMatrix* A, JXF_Int levfill, JXF_Real droptol, JXF_Real permtol, JXF_Int mbloc, JXF_Real* alu,
                                 JXF_Int* jlu, JXF_Int* iperm, JXF_Int iwk, JXF_Int* nzlu)
{
    JXF_Int      ierr   = JXF_SUCCESS;
    JXF_Int      row    = jxf_CSRMatrixNumRows(A);
    JXF_Int*     A_i    = jxf_CSRMatrixI(A);
    ...
    jxf_ilutp(row, A_data, A_j, A_i, levfill, droptol, permtol, mbloc, alu, jlu, iperm, iwk, &ierr, nzlu);
...
/**
 * \fn static void jxf_qsplit (JXF_Real *a, JXF_Int *ind, JXF_Int n, JXF_Int ncut)
 *
...
F161:
    mid    = first;
    abskey = jxf_abs(a[mid]);
    for (j = first + 1; j <= last; ++j) {
        if (jxf_abs(a[j]) > abskey) {
            ++mid;
            //     interchange
            tmp      = a[mid];
            itmp     = ind[mid];
...
/**
 * \fn static void jxf_sortrow (JXF_Int num,JXF_Int *q)
 *
...
static void jxf_sortrow(JXF_Int num, JXF_Int* q)
{
    ...
    JXF_Int key, icn, ih, ii, i, j, jj;
    JXF_Int iinc[6] = {0, 1, 4, 13, 40, 121};
...
/**
 * \fn static void jxf_check_col_index (JXF_Int row, JXF_Int num, JXF_Int *q)
 *
