                       ...
                       JXF_Int      *num_dof_procs )
{
   ...
   else if (!dof_partition && num_dof_procs)
   {
      ...
      dof_partition = fsls_CTAlloc(JXF_Int, nprocs+1);
      dof_partition[0] = 0;
      for (i = 0; i < nprocs; i ++)
      {
   ...
   else if (dof_partition && !num_dof_procs)
   {
      ...
      num_dof_procs = fsls_CTAlloc(JXF_Int, nprocs);
      for (i = 0; i < nprocs; i ++)
      {
...
/*!
 * \fn JXF_Int fsls_ParGLSDataInitialize
 ...
 */ 
fsls_ParGLSData *
fsls_GenerateGLSData0( MPI_Comm comm, JXF_Int global_num_ls, char *MatFile, char *RhsFile )
{
   ...
   JXF_Int global_num_dof; 
   JXF_Int num_rows_sub;
   JXF_Int num_cols_sub;
   JXF_Int num_nonzeros_sub;    
   JXF_Int i,myid,nprocs;
                      ...
                      JXF_Int             *partition )
{
   ...
   JXF_Int    *ia_sample = NULL;
   JXF_Int    *ja_sample = NULL;
   JXF_Real *a_sample  = NULL;
   
   JXF_Int begin,end;
   JXF_Int local_num_rows, num_nonzeros, local_num_rows1;
   JXF_Int j, myid;
   ...
   memcpy(ia_sample, &ia[begin],     (local_num_rows+1)*sizeof(JXF_Int));
   memcpy(ja_sample, &ja[ia[begin]], num_nonzeros*sizeof(JXF_Int));
   memcpy(a_sample,  &a[ia[begin]],  num_nonzeros*sizeof(JXF_Real));
                      ...
                      JXF_Int          *partition )
{
   ...
   memcpy(fsls_VectorData(sample), &x_data[begin], local_size*sizeof(JXF_Real) );
