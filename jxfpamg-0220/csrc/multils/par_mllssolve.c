...
/*!
 * \fn JXF_Int fsls_SolverParamSet**
 * \author peghoty
 * \date 2010/11/23 
 */
JXF_Int
fsls_SolverParamSetSolverID( void *vdata, JXF_Int solver_id )
{
   JXF_Int ierr = 0;
...
JXF_Int
fsls_SolverParamSetTolerance( void *vdata, JXF_Real tol )
{
   JXF_Int ierr = 0;
   fsls_SolverParam *solver = vdata;
...
JXF_Int
fsls_SolverParamSetMaxIter( void *vdata, JXF_Int max_iter )
{
   JXF_Int ierr = 0;
   fsls_SolverParam *solver = vdata;
...
JXF_Int
fsls_SolverParamSetMinIter( void *vdata, JXF_Int min_iter )
{
   JXF_Int ierr = 0;
   fsls_SolverParam *solver = vdata;
...
JXF_Int
fsls_SolverParamSetTwoNorm( void *vdata, JXF_Int two_norm )
{
   JXF_Int ierr = 0;
   fsls_SolverParam *solver = vdata;
...
JXF_Int
fsls_SolverParamSetKDim( void *vdata, JXF_Int k_dim )
{
   JXF_Int ierr = 0;
   fsls_SolverParam *solver = vdata;
...
JXF_Int
fsls_SolverParamSetPrintLevel( void *vdata, JXF_Int print_level )
{
   JXF_Int ierr = 0;
   fsls_SolverParam *solver = vdata;
...
fsls_ParallelSolve( fsls_ParLSData *ls_data, fsls_SolverParam *solver )
{
   ...
   JXF_Int    solver_id   = fsls_SolverParamSolverID(solver);    // solver id
   JXF_Real tol         = fsls_SolverParamTolerance(solver);   // the tolerance   
   JXF_Int    max_iter    = fsls_SolverParamMaxIter(solver);     // the maximal number of iterations
   JXF_Int    min_iter    = fsls_SolverParamMinIter(solver);     // the minimal number of iterations  
   JXF_Int    two_norm    = fsls_SolverParamTwoNorm(solver);     // L^2 norm(1) or preconditioner-norm(0)? used for PCG
   JXF_Int    k_dim       = fsls_SolverParamKDim(solver);        // the dimension of krylov subspace, used for PGMRES 
   JXF_Int    print_level = fsls_SolverParamPrintLevel(solver);  // how much information to be printed
   ...
   JXF_Real       strong_threshold;  // the strong threshold which defines the Strenth Matrix
   JXF_Real       max_row_sum;       // parameter to modify the definition of strength for diagonal 
   ...
   JXF_Real       A_trunc_factor;    // truncation parameter for coarse operater
   JXF_Real       P_trunc_factor;    // truncation parameter for interpolation operater
   JXF_Int          A_max_elmts;       // maximal number of entries in each row of coarse operater
   JXF_Int          P_max_elmts;       // maximal number of entries in each row of interpolation operater
   
   /* output information */
   JXF_Real       operat_cmplxty  = 0.0;
   JXF_Real       grid_cmplxty    = 0.0; 
   JXF_Real       last_rel_nrm    = 0.0;
   JXF_Real       conv_factor     = 0.0;
   JXF_Int          solver_iter     = 0;  
   ...
   *-----------------------------------------------------*/
   ...
   {
      if (print_level > 0)
      jxf_printf(" myid = \033[31m%d\033[00m solver: \033[31mAMG\033[00m \n", myid);
   ...
   {
      if (print_level > 0)
      jxf_printf(" myid = \033[31m%d\033[00m solver: \033[31mCG\033[00m \n", myid);
   ...
   {
      if (print_level > 0)
      jxf_printf(" myid = \033[31m%d\033[00m solver: \033[31mAMG-CG\033[00m \n", myid);
   ...
   {
      if (print_level > 0)
      jxf_printf(" myid = \033[31m%d\033[00m solver: \033[31mDS-CG\033[00m \n", myid);
   ...
   {
      if (print_level > 0)
      jxf_printf(" myid = \033[31m%d\033[00m solver: \033[31mGMRES\033[00m \n", myid);
   ...
   {
      if (print_level > 0)
      jxf_printf(" myid = \033[31m%d\033[00m solver: \033[31mAMG-GMRES\033[00m \n", myid);
   ...
   {
      if (print_level > 0)
      jxf_printf(" myid = \033[31m%d\033[00m solver: \033[31mDS-GMRES\033[00m \n", myid);
   ...
   {
      if (print_level > 0)
      jxf_printf(" myid = \033[31m%d\033[00m solver: \033[31mBiCGSTAB\033[00m \n", myid);
   ...
   {
      if (print_level > 0)
      jxf_printf(" myid = \033[31m%d\033[00m solver: \033[31mAMG-BiCGSTAB\033[00m \n", myid);
   ...
   {
      if (print_level > 0)
      jxf_printf(" myid = \033[31m%d\033[00m solver: \033[31mDS-BiCGSTAB\033[00m \n", myid);
