                ...
                jxf_ParVector    *u )
{
   ...
   /* Matrices */
   jxf_ParCSRMatrix   *matmL              = jxf_ParILUDataMatLModified(ilu_data);
   jxf_ParCSRMatrix   *matmU              = jxf_ParILUDataMatUModified(ilu_data);
...
//    jxf_CSRMatrix      *matALU_d           = jxf_ParILUDataMatAILUDevice(ilu_data);
//    jxf_CSRMatrix      *matBLU_d           = jxf_ParILUDataMatBILUDevice(ilu_data);
//    jxf_CSRMatrix      *matE_d             = jxf_ParILUDataMatEDevice(ilu_data);
//    jxf_CSRMatrix      *matF_d             = jxf_ParILUDataMatFDevice(ilu_data);
//    jxf_ParCSRMatrix   *Aperm              = jxf_ParILUDataAperm(ilu_data);
//    jxf_Vector         *Adiag_diag         = jxf_ParILUDataADiagDiag(ilu_data);
//    jxf_Vector         *Sdiag_diag         = jxf_ParILUDataSDiagDiag(ilu_data);
//    jxf_ParVector      *Ztemp              = jxf_ParILUDataZTemp(ilu_data);
//    JXF_Int             test_opt           = jxf_ParILUDataTestOption(ilu_data);
// #endif
...
JXF_Int
jxf_parJacobi_TriangularSolve(jxf_ParCSRMatrix *A,
                     jxf_ParVector    *f,
                     jxf_ParVector    *u,
                     JXF_Int           nLU,
                     jxf_ParCSRMatrix *L,
                     ...
                     JXF_Int           upper_jacobi_iters)
{
   ...
   // char FileNameCoaMat[256];
   //  jxf_sprintf(FileNameCoaMat, "A_CSR_%d", 4);
   //  jxf_ParCSRMatrixPrint(A, FileNameCoaMat);
   //  jxf_sprintf(FileNameCoaMat, "L_CSR_%d", 4);
   //  jxf_ParCSRMatrixPrint(L, FileNameCoaMat);
   //  jxf_sprintf(FileNameCoaMat, "U_CSR_%d", 4);
   //  jxf_ParCSRMatrixPrint(U, FileNameCoaMat);

   // jxf_sprintf(FileNameCoaMat, "  D_CSR_%d", 4);
   //  jxf_ParVectorPrint(D_vector, FileNameCoaMat);

   jxf_ParVectorCopy(f, ftemp);
   jxf_ParCSRMatrixMatvec(alpha, A, u, beta, ftemp);

   // jxf_ParVector *ytemp = jxf_ParVectorCreate(comm, jxf_ParVectorGlobalSize(u), jxf_ParVectorPartitioning(u));
   // jxf_ParVectorInitialize(ytemp);
   // jxf_ParVectorSetPartitioningOwner(ytemp,0);


   // jxf_ParVector *unew = jxf_ParVectorCreate(comm, jxf_ParVectorGlobalSize(u), jxf_ParVectorPartitioning(u));
   // jxf_ParVectorInitialize(unew);
   // jxf_ParVectorSetPartitioningOwner(unew,0);
   // jxf_ParVectorSetConstantValues(unew, 0);
   ...
   // JXF_Real  *utemp_data = jxf_VectorData(utemp_local);
    ...
    for (iter = 0; iter < maxIter; iter++) 
    {
      ...
      for ( kk = 0; kk < lower_jacobi_iters; kk++ )  //lower_jacobi_iters
      {
         jxf_ParCSRMatrixMatvec(alpha,L, ytemp, 0, utemp);
         // jxf_ParVectorAxpy(1,ftemp, utemp);
         #pragma omp parallel for
         for (i = 0; i < nLU; i++) 
         {
      ...
      for ( kk = 0; kk < upper_jacobi_iters; kk++ )  //upper_jacobi_iters
      {
         jxf_ParCSRMatrixMatvec(alpha,U, utemp, 0, unew);
         // jxf_ParVectorAxpy(1,ytemp, unew);
         #pragma omp parallel for
         for (i = 0; i < nLU; i++) 
         {
            utemp_data[i] = D_data[i] * (ytemp_data[i] + unew_data[i]);
         }
         // jxf_ParVecMul(D_vector,unew,utemp);
         // jxf_ParVectorCopy(utemp, ytemp);
      } /* end jacobi loop */
   ...
   // jxf_ParVectorDestroy(ytemp);
   // jxf_ParVectorDestroy(unew);   /* 修正：释放 unew，避免内存泄漏 */
