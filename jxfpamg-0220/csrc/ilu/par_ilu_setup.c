...
JXF_Int
jxf_ILUSetup(void *ilu_vdata, jxf_ParCSRMatrix *A, jxf_ParVector *f, jxf_ParVector *u)
{
    ...
    matU = NULL;
    jxf_ParCSRMatrixDestroy(matmL);
    matmL = NULL;
    jxf_ParCSRMatrixDestroy(matmU);
    matmU = NULL;
   ...
   jxf_ParILUDataYTemp(ilu_data) = Ytemp;
    ...
    switch (ilu_type)
    {
        ...
        case 3:
        {
            jxf_ILUSetupFGPILU_v3(matA, sweep, &matL, &D_array, &matU, &matS, &u_end);
        }
        break;
    }
...
{
   JXF_Int nx = 127; //(JXF_Int)(round(pow((double)nLU, 1.0 / 3.0)));
   JXF_Int ny = 127;
   JXF_Int nz = 127;
...
#pragma omp parallel num_threads(nt)
    ...
    if (num_procs > 1)
    {
        ...
        jxf_ParCSRMatrixM( matL,D_array );
    }
...
#pragma omp parallel num_threads(nt)
            {
                ...
                for (int r = istart; r < iend && r < n; r++)
                {
                    ...
                    earlier when we built L_offd_j/U_offd_j we used workL/workU offsets; we can reuse the same scheme:
                    */
                    // JXF_Int baseL = workL[tid];
                    // JXF_Int baseU = workU[tid];
...
JXF_Int
jxf_ParCSRMatrixM(  jxf_ParCSRMatrix *A,jxf_ParVector    *x )
{
    jxf_ParCSRCommHandle **comm_handle;
    ...
    JXF_Int n = jxf_CSRMatrixNumRows(diag);
    JXF_Int nt = jxf_NumThreads();
    JXF_Int ROW_OMP;
