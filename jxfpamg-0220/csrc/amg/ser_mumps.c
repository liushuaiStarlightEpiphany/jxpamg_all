...
jxf_Mumps_float( JXF_Solver *solve,jxf_ParCSRMatrix *pA, jxf_ParVector *pb, jxf_ParVector *px )
{
    ...
    jxf_CSRMatrix *A = jxf_ParCSRMatrixToCSRMatrixAll(pA);
...
JXF_Int mumps_Interface_float(MPI_Comm comm, int row, int nnz, int *ia, int  *ja, JXF_Real *aa, JXF_Real *x, JXF_Real *b)  
{
    int myid, num_procs;
    SMUMPS_STRUC_C id;
    const  int prtlvl = 1;
...
int mumps_Interface_double(MPI_Comm comm, int row, int nnz, int *ia, int  *ja, JXF_Real *aa, JXF_Real *x, JXF_Real *b)  
{
    int myid, num_procs;
    DMUMPS_STRUC_C id;
    const  int prtlvl = 1;
...
jxf_Mumps_double_solve(jxf_DMUMPS_data *mumps_data,jxf_ParCSRMatrix *pA,jxf_ParVector *pb, jxf_ParVector *px)
{
    ...
    JXF_Int n = mumps_data->num_rows;
    JXF_Int nz = mumps_data->num_nonzeros;
    // printf("%d seconds.\n", n);
    // printf("%d seconds.\n", nz);
