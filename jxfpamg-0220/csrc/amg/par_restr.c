...
#include "jxf_pamg.h"
#include "jxf_blas2.h"
#include "jxf_lapack2.h"

/*!
 * \fn JXF_Int jxf_PAMGBuildRestrAIR
                      ...
                      JXF_Int            *R_max_size_ptr )
{
   ...
   jxf_ParCSRMatrix *R;
   /* csr's */
   jxf_CSRMatrix *R_diag;
   jxf_CSRMatrix *R_offd;
   /* arrays */
   JXF_Real      *R_diag_data;
   JXF_Int       *R_diag_i;
   JXF_Int       *R_diag_j;
   JXF_Real      *R_offd_data;
   JXF_Int       *R_offd_i;
   JXF_Int       *R_offd_j;
   JXF_Int       *col_map_offd_R;
   ...
   JXF_Int        i, j, k, i1, k1, k2, rr, cc, ic, index, start, local_max_size, local_size, num_cols_offd_R;

   /* LAPACK */
   JXF_Real *DAi, *Dbi;
   JXF_Int *Ipi, lapack_info, ione = 1;
   char charT = 'T';
   ...
   JXF_Int n_cpts = 0;
   JXF_Int col_start = jxf_ParCSRMatrixFirstRowIndex(A);
   JXF_Int col_end   = col_start + n_fine;
   ...
   for (i = 0; i < n_fine; i++)
   {
      ...
      /* keep ths max size */
      local_max_size = jxf_max(local_max_size, local_size);
   }
   ...
   /*------------- allocate arrays */
   R_diag_i    = jxf_CTAlloc(JXF_Int,  n_cpts+1);
   R_diag_j    = jxf_CTAlloc(JXF_Int,  nnz_diag);
   R_diag_data = jxf_CTAlloc(JXF_Real, nnz_diag);

   /* not in ``if num_procs > 1'', 
    * allocation needed even for empty CSR */
   R_offd_i    = jxf_CTAlloc(JXF_Int,  n_cpts+1);
   R_offd_j    = jxf_CTAlloc(JXF_Int,  nnz_offd);
   R_offd_data = jxf_CTAlloc(JXF_Real, nnz_offd);
   ...
   /* matrix */
   DAi = jxf_CTAlloc(JXF_Real, local_max_size * local_max_size);
   /* rhs */
   Dbi = jxf_CTAlloc(JXF_Real, local_max_size);
   /* pivot */
   Ipi = jxf_CTAlloc(JXF_Int, local_max_size);
   ...
   for (i = 0, ic = 0; i < n_fine; i++)
   {
      ...
      /* clear DAi and bi */
      memset(DAi, 0, local_size * local_size * sizeof(JXF_Real));
      memset(Dbi, 0, local_size * sizeof(JXF_Real));
         ...
         if (CF_marker[i1] < 0)
         {
            ...
            if (num_procs > 1)
            {
               for (k = A_offd_i[i1]; k < A_offd_i[i1+1]; k++)
               {
                  ...
                  {
                     jxf_assert(CF_marker_offd[k1] < 0);
                     /* copy the value */
                     /* rr and cc: local dense ids */
            ...
            if (CF_marker_offd[i1] < 0)
            {
               ...
               for (k = A_ext_i[i1]; k < A_ext_i[i1+1]; k++)
               {
                  ...
                  else
                  {
                     ...
                     k2 = jxf_BinarySearch(col_map_offd_A, k1, num_cols_A_offd);
                     ...
                     if (k2 > -1)
                     {
                        ...
                        {
                           jxf_assert(CF_marker_offd[k2] < 0);
                           /* copy the value */
                           /* rr and cc: local dense ids */
                           DAi[rr + cc * local_size] = A_ext_data[k];
                        }
      ...
      if (local_size > 0)
      {
         ...
         jxf_dgetrf(&local_size, &local_size, DAi, &local_size, Ipi, &lapack_info);

         jxf_assert(lapack_info == 0);

         if (lapack_info == 0)
         {
            /* solve A_i^T x_i = b_i,
             * solution is saved in b_i on return */
            jxf_dgetrs(&charT, &local_size, &ione, DAi, &local_size, Ipi, Dbi, &local_size, &lapack_info);
            jxf_assert(lapack_info == 0);
         }
      }
   ...
   col_map_offd_R = jxf_CTAlloc(JXF_Int, num_cols_offd_R);
   ...
   {
      i1 = R_offd_j[i];
      k1 = jxf_BinarySearch(col_map_offd_R, i1, num_cols_offd_R);
      /* search must succeed */
      jxf_assert(k1 >= 0 && k1 < num_cols_offd_R);
      R_offd_j[i] = k1;
   }
   ...
   R = jxf_ParCSRMatrixCreate(comm,
                                total_global_cpts, /* global num of rows */
                                jxf_ParCSRMatrixGlobalNumRows(A), /* global num of cols */
                                num_cpts_global, /* row_starts */
                                jxf_ParCSRMatrixRowStarts(A), /* col_starts */
                                num_cols_offd_R, /* num cols offd */
                                nnz_diag,
                                nnz_offd);

   R_diag = jxf_ParCSRMatrixDiag(R);
   jxf_CSRMatrixData(R_diag) = R_diag_data;
   jxf_CSRMatrixI(R_diag)    = R_diag_i;
   jxf_CSRMatrixJ(R_diag)    = R_diag_j;

   R_offd = jxf_ParCSRMatrixOffd(R);
   jxf_CSRMatrixData(R_offd) = R_offd_data;
   jxf_CSRMatrixI(R_offd)    = R_offd_i;
   jxf_CSRMatrixJ(R_offd)    = R_offd_j;
   /* R does not own ColStarts, since A does */
   jxf_ParCSRMatrixOwnsColStarts(R) = 0;
   
   jxf_ParCSRMatrixColMapOffd(R) = col_map_offd_R;

   /* create CommPkg of R */
   jxf_MatvecCommPkgCreate(R);
...
/*!
 * \fn JXF_Int jxf_PAMGBuildRestrDist2AIR
                           ...
                           JXF_Int            *R_max_size_ptr )
{
   ...
   JXF_Int        i, j, j1, j2, k, i1, i2, k1, k2, k3, rr, cc, ic, index, start, end, local_max_size, local_size, num_cols_offd_R;
   /*JXF_Int        i6;*/
   JXF_Int        *FF2_offd, FF2_offd_len;

   /* LAPACK */
   JXF_Real *DAi, *Dbi;
   JXF_Int *Ipi, lapack_info, ione = 1, *RRi, *KKi;
   char charT = 'T';
   ...
   JXF_Int nnz_diag, nnz_offd, cnt_diag, cnt_offd;
   JXF_Int *Marker_diag, *Marker_offd;
   JXF_Int *Marker_diag_j, Marker_diag_count;
   JXF_Int num_sends, num_recvs, num_elems_send;
   /* local size, local num of C points */
   JXF_Int n_fine = jxf_CSRMatrixNumRows(A_diag);
   JXF_Int n_cpts = 0;
   JXF_Int col_start = jxf_ParCSRMatrixFirstRowIndex(A);
   ...
   JXF_Int *send_SF_i, *send_SF_j, send_SF_jlen;
   JXF_Int *recv_SF_i, *recv_SF_j, *recv_SF_j2, recv_SF_jlen;
   JXF_Int *send_SF_jstarts, *recv_SF_jstarts;
   JXF_Int *recv_SF_offd_list, recv_SF_offd_list_len;
   JXF_Int *Mapper_recv_SF_offd_list, *Mapper_offd_A, *Marker_recv_SF_offd_list;
   JXF_Int *Marker_FF2_offd;
   JXF_Int *Marker_FF2_offd_j, Marker_FF2_offd_count;

   /* for communication of offd F and F^2 rows of A */
   jxf_ParCSRCommPkg *comm_pkg_FF2_i, *comm_pkg_FF2_j;
   JXF_Int num_sends_FF2, *send_FF2_i, send_FF2_ilen, *send_FF2_j, send_FF2_jlen,
             num_recvs_FF2, *recv_FF2_i, recv_FF2_ilen, *recv_FF2_j, recv_FF2_jlen,
             *send_FF2_jstarts, *recv_FF2_jstarts;
   JXF_Real *send_FF2_a, *recv_FF2_a;

   /* ghost rows: offd F and F2-pts */
   jxf_CSRMatrix *A_offd_FF2   = NULL;
   ...
   num_elems_send = jxf_ParCSRCommPkgSendMapStart(comm_pkg, num_sends);

   /* send buffer, of size send_map_starts[num_sends]), 
    * i.e., number of entries to send */
   send_buf_i = jxf_CTAlloc(JXF_Int , num_elems_send);
   ...
   for (i = 0, index = 0; i < num_sends; i++)
   {
      ...
      {
         /* CF marker of send_map_elemts[j] */
         send_buf_i[index++] = CF_marker[jxf_ParCSRCommPkgSendMapElmt(comm_pkg,j)];
      }
   }
   /* create a handle to start communication. 11: for integer */
   comm_handle = jxf_ParCSRCommHandleCreate(11, comm_pkg, send_buf_i, CF_marker_offd);
   /* destroy the handle to finish communication */
   jxf_ParCSRCommHandleDestroy(comm_handle);
   ...
   if (num_functions > 1)
   {
      for (i = 0, index = 0; i < num_sends; i++)
      {
         ...
         for (j=start; j < jxf_ParCSRCommPkgSendMapStart(comm_pkg, i+1); j++)
         {
            send_buf_i[index++] = dof_func[jxf_ParCSRCommPkgSendMapElmt(comm_pkg,j)];
         }
      }
      comm_handle = jxf_ParCSRCommHandleCreate(11, comm_pkg, send_buf_i, dof_func_offd);
      jxf_ParCSRCommHandleDestroy(comm_handle);   
   }
    ...
    *        F^2: OffdF - F
    *- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
   send_SF_i = jxf_CTAlloc(JXF_Int, num_elems_send);
   recv_SF_i = jxf_CTAlloc(JXF_Int, num_cols_A_offd + 1);
   ...
   for (i = 0, send_SF_jlen = 0; i < num_elems_send; i++)
   {
      ...
      send_SF_i[i] = 0;
      /* elem i1 */
      i1 = jxf_ParCSRCommPkgSendMapElmt(comm_pkg, i);
      /* ignore C-pts */
      if (CF_marker[i1] >= 0)
      {
         continue;
   ...
   /* do communication */
   comm_handle = jxf_ParCSRCommHandleCreate(11, comm_pkg, send_SF_i, recv_SF_i+1);
   /* ... */
   jxf_ParCSRCommHandleDestroy(comm_handle);
   
   send_SF_j = jxf_CTAlloc(JXF_Int, send_SF_jlen);
   send_SF_jstarts = jxf_CTAlloc(JXF_Int, num_sends + 1);
   ...
   for (i = 0, i1 = 0; i < num_sends; i++)
   {
      ...
      {
         /* strong F-pt, j1 */
         j1 = jxf_ParCSRCommPkgSendMapElmt(comm_pkg, j);
         /* ignore C-pts */
         if (CF_marker[j1] >= 0)
         {
            continue;
   ...
   recv_SF_j = jxf_CTAlloc(JXF_Int, recv_SF_jlen);
   recv_SF_jstarts = jxf_CTAlloc(JXF_Int, num_recvs + 1);

   for (i = 1; i <= num_recvs; i++)
   {
      start = jxf_ParCSRCommPkgRecvVecStart(comm_pkg, i);
      recv_SF_jstarts[i] = recv_SF_i[start];
   }

   /* create a communication package for SF_j */
   comm_pkg_SF = jxf_CTAlloc(jxf_ParCSRCommPkg, 1);
   jxf_ParCSRCommPkgComm         (comm_pkg_SF) = comm;
   jxf_ParCSRCommPkgNumSends     (comm_pkg_SF) = num_sends;
   jxf_ParCSRCommPkgSendProcs    (comm_pkg_SF) = jxf_ParCSRCommPkgSendProcs(comm_pkg);
   jxf_ParCSRCommPkgSendMapStarts(comm_pkg_SF) = send_SF_jstarts;
   jxf_ParCSRCommPkgNumRecvs     (comm_pkg_SF) = num_recvs;
   jxf_ParCSRCommPkgRecvProcs    (comm_pkg_SF) = jxf_ParCSRCommPkgRecvProcs(comm_pkg);
   jxf_ParCSRCommPkgRecvVecStarts(comm_pkg_SF) = recv_SF_jstarts;

   /* do communication */
   comm_handle = jxf_ParCSRCommHandleCreate(11, comm_pkg_SF, send_SF_j, recv_SF_j);
   /* ... */
   jxf_ParCSRCommHandleDestroy(comm_handle);
    ...
    *- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
   recv_SF_offd_list = jxf_CTAlloc(JXF_Int, recv_SF_jlen);
   ...
   jxf_qsort0(recv_SF_offd_list, 0, j-1);
   ...
   recv_SF_j2 = jxf_CTAlloc(JXF_Int, recv_SF_jlen);
   for (i = 0; i < recv_SF_jlen; i++)
   {
      i1 = recv_SF_j[i];
      if (i1 < col_start || i1 >= col_end)
      {
         j = jxf_BinarySearch(recv_SF_offd_list, i1, recv_SF_offd_list_len);
         jxf_assert(j >= 0 && j < recv_SF_offd_list_len);
         recv_SF_j2[i] = j;
      }
      else
      {
   ...
   Mapper_recv_SF_offd_list = jxf_CTAlloc(JXF_Int, recv_SF_offd_list_len);
   Marker_recv_SF_offd_list = jxf_CTAlloc(JXF_Int, recv_SF_offd_list_len);
   ...
   for (i = 0; i < recv_SF_offd_list_len; i++)
   {
      ...
      jxf_assert(i1 < col_start || i1 >= col_end);
      j = jxf_BinarySearch(col_map_offd_A, i1, num_cols_A_offd);
         ...
         if (CF_marker_offd[j1] < 0)
         {
            ...
            for (k = recv_SF_i[j1]; k < recv_SF_i[j1+1]; k++)
            {
               ...
               if (k1 < col_start || k1 >= col_end)
               {
                  ...
                  jxf_assert(recv_SF_offd_list[k2] == k1);
   ...
   FF2_offd = jxf_CTAlloc(JXF_Int, FF2_offd_len);
   ...
   for (i = 0; i < recv_SF_offd_list_len; i++)
   {
      ...
      {
         jxf_assert(Marker_recv_SF_offd_list[i] == 0);
      }

      if (Marker_recv_SF_offd_list[i])
      {
         j = recv_SF_offd_list[i];
         jxf_assert(j < col_start || j >= col_end);
         FF2_offd[k++] = j;
         Marker_recv_SF_offd_list[i] = 0;
      }
   }
   jxf_assert(k == FF2_offd_len);

   /* sort the list */
   jxf_qsort0(FF2_offd, 0, FF2_offd_len-1);

   /* there must be no repetition in FF2_offd */
   for (i = 1; i < FF2_offd_len; i++)
   {
      jxf_assert(FF2_offd[i] != FF2_offd[i-1]);
   }
   ...
   jxf_ParCSRFindExtendCommPkg(A, FF2_offd_len, FF2_offd, &comm_pkg_FF2_i);
   /* number of sends (#procs) */
   num_sends_FF2 = jxf_ParCSRCommPkgNumSends(comm_pkg_FF2_i);
   /* number of rows to send */
   send_FF2_ilen = jxf_ParCSRCommPkgSendMapStart(comm_pkg_FF2_i, num_sends_FF2);
   /* number of recvs (#procs) */
   num_recvs_FF2 = jxf_ParCSRCommPkgNumRecvs(comm_pkg_FF2_i);
   /* number of rows to recv */
   recv_FF2_ilen = jxf_ParCSRCommPkgRecvVecStart(comm_pkg_FF2_i, num_recvs_FF2);

   jxf_assert(FF2_offd_len == recv_FF2_ilen);

   send_FF2_i = jxf_CTAlloc(JXF_Int, send_FF2_ilen);
   recv_FF2_i = jxf_CTAlloc(JXF_Int, recv_FF2_ilen + 1);
   for (i = 0, send_FF2_jlen = 0; i < send_FF2_ilen; i++)
   {
      j = jxf_ParCSRCommPkgSendMapElmt(comm_pkg_FF2_i, i);
      send_FF2_i[i] = A_diag_i[j+1] - A_diag_i[j] + A_offd_i[j+1] - A_offd_i[j];
      send_FF2_jlen += send_FF2_i[i];
   }
 
   /* do communication */
   comm_handle = jxf_ParCSRCommHandleCreate(11, comm_pkg_FF2_i, send_FF2_i, recv_FF2_i+1);
   /* ... */
   jxf_ParCSRCommHandleDestroy(comm_handle);

   send_FF2_j = jxf_CTAlloc(JXF_Int, send_FF2_jlen);
   send_FF2_a = jxf_CTAlloc(JXF_Real, send_FF2_jlen);
   send_FF2_jstarts = jxf_CTAlloc(JXF_Int, num_sends_FF2 + 1);
   
   for (i = 0, i1 = 0; i < num_sends_FF2; i++)
   {
      start = jxf_ParCSRCommPkgSendMapStart(comm_pkg_FF2_i, i);
      end   = jxf_ParCSRCommPkgSendMapStart(comm_pkg_FF2_i, i+1);
      for (j = start; j < end; j++)
      {
         /* will send row j1 to send_proc[i] */
         j1 = jxf_ParCSRCommPkgSendMapElmt(comm_pkg_FF2_i, j);
   ...
   recv_FF2_j = jxf_CTAlloc(JXF_Int, recv_FF2_jlen);
   recv_FF2_a = jxf_CTAlloc(JXF_Real, recv_FF2_jlen);
   recv_FF2_jstarts = jxf_CTAlloc(JXF_Int, num_recvs_FF2 + 1);

   for (i = 1; i <= num_recvs_FF2; i++)
   {
      start = jxf_ParCSRCommPkgRecvVecStart(comm_pkg_FF2_i, i);
      recv_FF2_jstarts[i] = recv_FF2_i[start];
   }

   /* create a communication package for FF2_j */
   comm_pkg_FF2_j = jxf_CTAlloc(jxf_ParCSRCommPkg, 1);
   jxf_ParCSRCommPkgComm         (comm_pkg_FF2_j) = comm;
   jxf_ParCSRCommPkgNumSends     (comm_pkg_FF2_j) = num_sends_FF2;
   jxf_ParCSRCommPkgSendProcs    (comm_pkg_FF2_j) = jxf_ParCSRCommPkgSendProcs(comm_pkg_FF2_i);
   jxf_ParCSRCommPkgSendMapStarts(comm_pkg_FF2_j) = send_FF2_jstarts;
   jxf_ParCSRCommPkgNumRecvs     (comm_pkg_FF2_j) = num_recvs_FF2;
   jxf_ParCSRCommPkgRecvProcs    (comm_pkg_FF2_j) = jxf_ParCSRCommPkgRecvProcs(comm_pkg_FF2_i);
   jxf_ParCSRCommPkgRecvVecStarts(comm_pkg_FF2_j) = recv_FF2_jstarts;
   
   /* do communication */
   /* ja */
   comm_handle = jxf_ParCSRCommHandleCreate(11, comm_pkg_FF2_j, send_FF2_j, recv_FF2_j);
   /* ... */
   jxf_ParCSRCommHandleDestroy(comm_handle);
  
   /* a */
   comm_handle = jxf_ParCSRCommHandleCreate( 1, comm_pkg_FF2_j, send_FF2_a, recv_FF2_a);
   /* ... */
   jxf_ParCSRCommHandleDestroy(comm_handle);

   /* A_offd_FF2 is ready ! */
   A_offd_FF2 = jxf_CSRMatrixCreate(recv_FF2_ilen, jxf_ParCSRMatrixGlobalNumCols(A),
                                      recv_FF2_jlen);

   jxf_CSRMatrixI   (A_offd_FF2) = recv_FF2_i;
   jxf_CSRMatrixJ   (A_offd_FF2) = recv_FF2_j;
   jxf_CSRMatrixData(A_offd_FF2) = recv_FF2_a;
   ...
   Mapper_offd_A = jxf_CTAlloc(JXF_Int, num_cols_A_offd);
   for (i = 0; i < num_cols_A_offd; i++)
   {
      Mapper_offd_A[i] = jxf_BinarySearch(FF2_offd, col_map_offd_A[i], FF2_offd_len);
   }
   
   /* Mapping from recv_SF_offd_list, overwrite the old one*/
   for (i = 0; i < recv_SF_offd_list_len; i++)
   {
      Mapper_recv_SF_offd_list[i] = jxf_BinarySearch(FF2_offd, recv_SF_offd_list[i], FF2_offd_len);
   }

   /* marker */
   Marker_FF2_offd = jxf_CTAlloc(JXF_Int, FF2_offd_len);
   ...
   for (i = 0; i < n_fine; i++)
   {
      ...
      for (j = S_diag_i[i]; j < S_diag_i[i+1]; j++)
      {
         ...
         for (k = S_offd_i[j1]; k < S_offd_i[j1+1]; k++)
         {
            ...
            if (CF_marker_offd[k1] < 0)
            {
               ...
               jxf_assert(k2 >= 0 && k2 < FF2_offd_len);
      ...
      for (j = S_offd_i[i]; j < S_offd_i[i+1]; j++)
      {
         ...
         /* this mapping must be successful */
         jxf_assert(j2 >= 0 && j2 < FF2_offd_len);
         ...
         for (k = recv_SF_i[j1]; k < recv_SF_i[j1+1]; k++)
         {
            ...
            else /* k1 is in the offd part */
            {
               ...
               /* this mapping must be successful */
               jxf_assert(k3 >= 0 && k3 < FF2_offd_len);
   ...
   Marker_diag_j = jxf_CTAlloc(JXF_Int, n_fine);

   for (i = 0; i < FF2_offd_len; i++)
   {
      Marker_FF2_offd[i] = -1;
   }
   Marker_FF2_offd_j = jxf_CTAlloc(JXF_Int, FF2_offd_len);
    ...
    *- - - - - - - - - - - - - - - - - - - - - - - - - */
   RRi = jxf_CTAlloc(JXF_Int, local_max_size);
   /* indicators for RRi of being local (0) or offd (1) */
   KKi = jxf_CTAlloc(JXF_Int, local_max_size);
...
//time1 += t1;
      ...
      jxf_assert(local_size <= local_max_size);
...
//JXF_Real t2 = jxf_MPI_Wtime();
      ...
      for (rr = 0; rr < local_size; rr++)
      {
         ...
         if (i2)  /* i2 == 1, i1 is an offd row */
         {
            ...
            for (j = jxf_CSRMatrixI(A_offd_FF2)[i1]; j < jxf_CSRMatrixI(A_offd_FF2)[i1+1]; j++)
            {
               /* j1 is a global index */
               j1 = jxf_CSRMatrixJ(A_offd_FF2)[j];
               ...
               if (j1 >= col_start && j1 < col_end)
               {
                  ...
                  {
                     jxf_assert(CF_marker[j2] < 0);
                     /* copy the value */
                     /* rr and cc: local dense ids */
                     DAi[rr + cc * local_size] = jxf_CSRMatrixData(A_offd_FF2)[j];
                  }
               }
               else
               {
                  /* j1 is in offd part, search it in FF2_offd */
                  j2 =  jxf_BinarySearch(FF2_offd, j1, FF2_offd_len);
                  /* if found */
                  if (j2 > -1)
                  {
                     /* if this col is marked with its local dense id */
            ...
            if (num_procs > 1)
            {
               for (j = A_offd_i[i1]; j < A_offd_i[i1+1]; j++)
               {
                  ...
                  if (j2 > -1)
                  {
                     ...
                     if ((cc = Marker_FF2_offd[j2]) >= 0)
                     {
                        jxf_assert(CF_marker_offd[j1] < 0);
                        /* copy the value */
                        /* rr and cc: local dense ids */
...
//time2 += t2;
      ...
      for (rr = 0; rr < local_size; rr++)
      {
         ...
         if (i2) /* offd */
         {
            jxf_assert(Marker_FF2_offd[i1] == rr);
...
//JXF_Real t3 = jxf_MPI_Wtime();
      ...
      for (j = 0; j < n_fine; j++)
      {
         jxf_assert(Marker_diag[j] == -1);
      }

      for (j = 0; j < FF2_offd_len; j++)
      {
         jxf_assert(Marker_FF2_offd[j] == -1);
      }
      */
...
//time3 += t3;
   ...
   for (i = 0; i < FF2_offd_len; i++)
   {
      jxf_assert(Marker_FF2_offd[i] == - 1);
   }
   */
  ...
  *R_max_size_ptr = local_max_size;
   ...
   jxf_TFree(recv_SF_i);
   jxf_TFree(send_SF_j);
   jxf_TFree(send_SF_jstarts);
   jxf_TFree(recv_SF_j);
   jxf_TFree(recv_SF_jstarts);
   jxf_TFree(comm_pkg_SF);
   jxf_TFree(recv_SF_offd_list);
   jxf_TFree(recv_SF_j2);
   jxf_TFree(Mapper_recv_SF_offd_list);
   jxf_TFree(Marker_recv_SF_offd_list);
   jxf_TFree(FF2_offd);
   jxf_TFree(send_FF2_i);
   /* jxf_TFree(recv_FF2_i); */
   jxf_TFree(send_FF2_j);
   jxf_TFree(send_FF2_a);
   jxf_TFree(send_FF2_jstarts);
   /* jxf_TFree(recv_FF2_j); */
   /* jxf_TFree(recv_FF2_a); */
   jxf_CSRMatrixDestroy(A_offd_FF2);
   jxf_TFree(recv_FF2_jstarts);
   jxf_MatvecCommPkgDestroy(comm_pkg_FF2_i);
   jxf_TFree(comm_pkg_FF2_j);
   jxf_TFree(Mapper_offd_A);
   jxf_TFree(Marker_FF2_offd);
   jxf_TFree(Marker_diag_j);
   jxf_TFree(Marker_FF2_offd_j);
   jxf_TFree(DAi);
   jxf_TFree(Dbi);
   jxf_TFree(Ipi);
   jxf_TFree(RRi);
