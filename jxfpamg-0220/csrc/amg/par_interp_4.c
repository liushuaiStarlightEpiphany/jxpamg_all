...
/*!
 * \fn JXF_Int jxf_PAMGBuildMultipass
                       ...
                       jxf_ParCSRMatrix **P_ptr )
{
   ...
   JXF_Int             *P_diag_i; /* at first counter of nonzero cols for each row,
				finally will be pointer to start of row */
   JXF_Int             *P_diag_j;

   jxf_CSRMatrix    *P_offd;
   JXF_Real          *P_offd_data = NULL;
   JXF_Int             *P_offd_i; /* at first counter of nonzero cols for each row,
		                 finally will be pointer to start of row */
   JXF_Int             *P_offd_j = NULL;

   JXF_Int              num_sends = 0;
   JXF_Int             *int_buf_data = NULL;
   JXF_Int             *send_map_start = NULL;
   JXF_Int             *send_map_elmt = NULL;
   JXF_Int             *send_procs = NULL;
   JXF_Int              num_recvs = 0;
   JXF_Int             *recv_vec_start = NULL;
   JXF_Int             *recv_procs = NULL;
   JXF_Int             *new_recv_vec_start = NULL;
   JXF_Int            **Pext_send_map_start = NULL;
   JXF_Int            **Pext_recv_vec_start = NULL;
   JXF_Int             *Pext_start = NULL;
   JXF_Int             *P_ncols = NULL;
   
   JXF_Int             *CF_marker_offd = NULL;
   JXF_Int             *dof_func_offd = NULL;
   JXF_Int             *P_marker;
   JXF_Int             *P_marker_offd = NULL;
   JXF_Int             *C_array = NULL;
   JXF_Int             *C_array_offd = NULL;
   JXF_Int             *pass_array = NULL;   /* contains points ordered according to pass */
   JXF_Int             *pass_pointer = NULL; /* pass_pointer[j] contains pointer to first
				            point of pass j contained in pass_array */
   JXF_Int             *P_diag_start;
   JXF_Int             *P_offd_start = NULL;
   JXF_Int            **P_diag_pass;
   JXF_Int            **P_offd_pass = NULL;
   JXF_Int            **Pext_pass = NULL;
   JXF_Int            **new_elmts = NULL;   /* new neighbors generated in each pass */
   JXF_Int             *new_counter = NULL; /* contains no. of new neighbors for each pass */
   JXF_Int             *loc = NULL;         /* contains locations for new neighbor 
			                   connections in int_o_buffer to avoid searching */
   JXF_Int             *Pext_i = NULL;      /* contains P_diag_i and P_offd_i info for nonzero
				           cols of off proc neighbors */
   JXF_Int             *Pext_send_buffer = NULL; /* used to collect global nonzero
				                col ids in P_diag for send_map_elmts */

   JXF_Int             *map_S_to_new = NULL;
   JXF_Int             *map_A_to_S = NULL;
   JXF_Int             *new_col_map_offd = NULL;
   JXF_Int             *col_map_offd_P = NULL;
   JXF_Int             *permute = NULL;
   ...
   JXF_Int              pass;
   JXF_Int              num_passes;
   JXF_Int              max_num_passes = 10;

   JXF_Int              n_fine;
   JXF_Int              n_coarse = 0;
   JXF_Int              n_coarse_offd = 0;
   JXF_Int              n_SF = 0;
   JXF_Int              n_SF_offd = 0;
   ...
   JXF_Int             *assigned = NULL;
   JXF_Int             *assigned_offd = NULL;

   JXF_Real          *Pext_send_data = NULL;
   JXF_Real          *Pext_data = NULL;

   JXF_Real           sum_C, sum_N;
   JXF_Real           sum_C_pos, sum_C_neg;
   JXF_Real           sum_N_pos, sum_N_neg;
   JXF_Real           diagonal;
   JXF_Real           alfa = 1.0;
   JXF_Real           beta = 1.0;
   JXF_Int              j_start;
   ...
   JXF_Int              j, j1;
   JXF_Int              k, k1, k2, k3;
   JXF_Int              pass_array_size;
   JXF_Int              global_pass_array_size;
   JXF_Int              local_pass_array_size;
   JXF_Int              my_id, num_procs;
   JXF_Int              index, start;
   JXF_Int              my_first_cpt;
   JXF_Int              total_global_cpts;
   JXF_Int              p_cnt;
   JXF_Int              total_nz_offd;
   JXF_Int              cnt_nz_offd;
   JXF_Int              cnt_offd, cnt_new;
   JXF_Int              no_break;
   JXF_Int              not_found;
   JXF_Int              Pext_send_size;
   JXF_Int              Pext_recv_size;
   JXF_Int              old_Pext_send_size;
   JXF_Int              old_Pext_recv_size;
   JXF_Int              P_offd_size = 0;
   JXF_Int              local_index = -1;
   JXF_Int              new_num_cols_offd = 0;
   ...
   *-----------------------------------------------------------------------*/
   ...
   n_fine = jxf_CSRMatrixNumRows(A_diag);
   ...
   *-----------------------------------------------------------------------*/
   ...
   if (pass_array_size) 
   {
      pass_array = jxf_CTAlloc(JXF_Int, pass_array_size);
   }
   pass_pointer = jxf_CTAlloc(JXF_Int, max_num_passes+1);
   if (n_fine) 
   {
      assigned = jxf_CTAlloc(JXF_Int, n_fine);
   }
   P_diag_i = jxf_CTAlloc(JXF_Int, n_fine + 1);
   P_offd_i = jxf_CTAlloc(JXF_Int, n_fine + 1);
   if (n_coarse) 
   {
      C_array = jxf_CTAlloc(JXF_Int, n_coarse);
   }
   ...
   if (num_procs > 1)
   {
      ...
      send_map_start = jxf_ParCSRCommPkgSendMapStarts(comm_pkg);
      send_map_elmt = jxf_ParCSRCommPkgSendMapElmts(comm_pkg);
      num_recvs = jxf_ParCSRCommPkgNumRecvs(comm_pkg);
      recv_procs = jxf_ParCSRCommPkgRecvProcs(comm_pkg);
      recv_vec_start = jxf_ParCSRCommPkgRecvVecStarts(comm_pkg);
      if (send_map_start[num_sends])
      {
         int_buf_data = jxf_CTAlloc(JXF_Int, send_map_start[num_sends]);
      }
   }
   ...
   if (num_cols_offd)
   {
      assigned_offd = jxf_CTAlloc(JXF_Int, num_cols_offd);
      map_S_to_new = jxf_CTAlloc(JXF_Int, num_cols_offd);
      fine_to_coarse_offd = jxf_CTAlloc(JXF_Int, num_cols_offd);
      new_col_map_offd = jxf_CTAlloc(JXF_Int, n_coarse_offd);
   }
   ...
   *-----------------------------------------------------------------------*/
   ...
   new_recv_vec_start = jxf_CTAlloc(JXF_Int, num_recvs + 1);

   if (n_coarse_offd)
   {
      C_array_offd = jxf_CTAlloc(JXF_Int,n_coarse_offd);
   }
   ...
   if (col_offd_S_to_A)
   {
      map_A_to_S = jxf_CTAlloc(JXF_Int, num_cols_offd_A);
   ...
   *-----------------------------------------------------------------------*/
   ...
   if (num_procs > 1)
   {
      comm_handle = jxf_ParCSRCommHandleCreate(11, comm_pkg, int_buf_data, assigned_offd);
      jxf_ParCSRCommHandleDestroy(comm_handle);
   }
   ...
   *-----------------------------------------------------------------------*/
   ...
   jxf_MPI_Allreduce(&local_pass_array_size, &global_pass_array_size, 1, JXF_MPI_INT, MPI_SUM, comm);
   ...
   while (global_pass_array_size && pass < max_num_passes)
   {
     	 ...
     	 if (no_break)
	 {
      ...
      /* jxf_printf("pass %d  remaining points %d \n", pass, local_pass_array_size); */
   ...
   P_marker = jxf_CTAlloc(JXF_Int, n_coarse); /* marks points to see if they have
						   been counted */
   ...
   if (n_coarse_offd)
   {
      P_marker_offd = jxf_CTAlloc(JXF_Int, n_coarse_offd);
   ...
   P_diag_pass = jxf_CTAlloc(JXF_Int*, num_passes);  /* P_diag_pass[i] will contain
				                   all column numbers for points of pass i */

   P_diag_pass[1] = jxf_CTAlloc(JXF_Int, cnt_nz);

   P_diag_start = jxf_CTAlloc(JXF_Int, n_fine); /* P_diag_start[i] contains
	                                      pointer to begin of column numbers in P_pass for point i,
	                                      P_diag_i[i+1] contains number of columns for point i */

   P_offd_start = jxf_CTAlloc(JXF_Int, n_fine);

   if (num_procs > 1)
   {
      P_offd_pass = jxf_CTAlloc(JXF_Int*, num_passes);

      if (cnt_nz_offd)
      {
         P_offd_pass[1] = jxf_CTAlloc(JXF_Int, cnt_nz_offd);
      }
      else
      {
         P_offd_pass[1] = NULL;
      ...
      new_elmts = jxf_CTAlloc(JXF_Int*, num_passes);

      new_counter = jxf_CTAlloc(JXF_Int, num_passes+1);
   ...
   *-----------------------------------------------------------------------*/
   ...
   if (num_procs > 1)
   {
      ...
      Pext_send_map_start = jxf_CTAlloc(JXF_Int*, num_passes);
      Pext_recv_vec_start = jxf_CTAlloc(JXF_Int*, num_passes);
      Pext_pass = jxf_CTAlloc(JXF_Int*, num_passes);
      Pext_i = jxf_CTAlloc(JXF_Int, num_cols_offd + 1);
      
      if (num_cols_offd) 
      {
         Pext_start = jxf_CTAlloc(JXF_Int, num_cols_offd);
      }
      
      if (send_map_start[num_sends])
      {
         P_ncols = jxf_CTAlloc(JXF_Int, send_map_start[num_sends]);
      }
   ...
   for (pass = 2; pass < num_passes; pass ++)
   {
      if (num_procs > 1)
      {
         Pext_send_map_start[pass] = jxf_CTAlloc(JXF_Int, num_sends + 1);
         Pext_recv_vec_start[pass] = jxf_CTAlloc(JXF_Int, num_recvs + 1);
         Pext_send_size = 0;
         ...
         comm_handle = jxf_ParCSRCommHandleCreate (11, comm_pkg, P_ncols, &Pext_i[1]);
         jxf_ParCSRCommHandleDestroy(comm_handle);

         if (Pext_send_size > old_Pext_send_size)
         {
            jxf_TFree(Pext_send_buffer);
            Pext_send_buffer = jxf_CTAlloc(JXF_Int, Pext_send_size);
         }
      ...
      if (num_procs > 1)
      {
         ...
         jxf_ParCSRCommPkgSendMapStarts(tmp_comm_pkg) = Pext_send_map_start[pass];
         jxf_ParCSRCommPkgNumRecvs(tmp_comm_pkg) = num_recvs;
         jxf_ParCSRCommPkgRecvProcs(tmp_comm_pkg) = recv_procs;
         jxf_ParCSRCommPkgRecvVecStarts(tmp_comm_pkg) = Pext_recv_vec_start[pass];

         if (Pext_recv_size)
         {
            Pext_pass[pass] = jxf_CTAlloc(JXF_Int, Pext_recv_size);
            new_elmts[pass-1] = jxf_CTAlloc(JXF_Int, Pext_recv_size);
         }
         else
         {
            Pext_pass[pass] = NULL;
         ...
         comm_handle = jxf_ParCSRCommHandleCreate(11, tmp_comm_pkg, Pext_send_buffer, Pext_pass[pass]);
         jxf_ParCSRCommHandleDestroy(comm_handle);

         if (Pext_recv_size > old_Pext_recv_size)
         {
            jxf_TFree(loc);
            loc = jxf_CTAlloc(JXF_Int, Pext_recv_size);
         }
         ...
         for (j = recv_vec_start[i]; j < recv_vec_start[i+1]; j ++)
	 {
	          ...
	          if (k2 > -1 && k2 < n_coarse)
	             ...
	             Pext_pass[pass][j1] = - k2 - 1;
                  ...
                  else
                  {
                     ...
                     {
                        k2 = jxf_BinarySearch(new_elmts[k3], k1, (new_counter[k3+1]-new_counter[k3]));
                        if (k2 > -1)
                        {
      ...
      if (cnt_new)
      {
  	 jxf_qsort2i(new_elmts[pass-1], loc, 0, cnt_new - 1);
         cnt = 0;
	 ...
	 Pext_pass[pass][loc[0]] = local_index;
      ...
      if (new_num_cols_offd < local_index + 1)
      {
         ...
         P_marker_offd = jxf_CTAlloc(JXF_Int, new_num_cols_offd);
	 ...
	 for (j = S_offd_i[i1]; j < S_offd_i[i1+1]; j ++)
	 {
      ...
      P_diag_pass[pass] = jxf_CTAlloc(JXF_Int, cnt_nz);
      if (cnt_nz_offd)
      {
         P_offd_pass[pass] = jxf_CTAlloc(JXF_Int, cnt_nz_offd);
      }
      else if (num_procs > 1)
      {
   ...
   jxf_TFree(P_ncols);
   jxf_TFree(Pext_send_buffer);
   jxf_TFree(new_recv_vec_start);
   jxf_TFree(P_marker);
   jxf_TFree(P_marker_offd);
   P_marker_offd = NULL;

   P_diag_j = jxf_CTAlloc(JXF_Int, total_nz);
   P_diag_data = jxf_CTAlloc(JXF_Real, total_nz);

   if (total_nz_offd)
   {
      P_offd_j = jxf_CTAlloc(JXF_Int, total_nz_offd);
      P_offd_data = jxf_CTAlloc(JXF_Real, total_nz_offd);
   }
   ...
   if (num_cols_offd)
   {
      P_marker_offd = jxf_CTAlloc(JXF_Int, num_cols_offd);
      for (i = 0; i < num_cols_offd; i ++)
      {
         P_marker_offd[i] = -1;
      }
		        ...
		        accordingly */
   {
      ...
      jxf_TFree(P_diag_pass[1]);
      if (num_procs > 1) 
      {
         jxf_TFree(P_offd_pass[1]);
      }
	  
      if (new_num_cols_offd > n_coarse_offd)
      {
         jxf_TFree(C_array_offd);
         C_array_offd = jxf_CTAlloc(JXF_Int, new_num_cols_offd);
      }
      ...
      for (pass = 2; pass < num_passes; pass ++)
      {
         ...
         if (num_procs > 1)
         {
            ...
            if (Pext_send_size > old_Pext_send_size)
            {
               jxf_TFree(Pext_send_data);
               Pext_send_data = jxf_CTAlloc(JXF_Real, Pext_send_size);
            }
            old_Pext_send_size = Pext_send_size;
            ...
            {
               jxf_TFree(Pext_data);
               Pext_data = jxf_CTAlloc(JXF_Real, Pext_recv_size);
            }
            old_Pext_recv_size = Pext_recv_size;

            comm_handle = jxf_ParCSRCommHandleCreate(1, tmp_comm_pkg, Pext_send_data, Pext_data);
            jxf_ParCSRCommHandleDestroy(comm_handle);

            jxf_TFree(Pext_send_map_start[pass]);
            jxf_TFree(Pext_recv_vec_start[pass]);
         }
         ...
         jxf_TFree(P_diag_pass[pass]);
         if (num_procs > 1)
         {
            jxf_TFree(P_offd_pass[pass]);
            jxf_TFree(Pext_pass[pass]);
         }
      }
   }
   ...
   jxf_TFree(Pext_send_map_start);
   jxf_TFree(Pext_recv_vec_start);
   if (n_coarse) 
   {
      jxf_TFree(C_array);
   }
   ...
   jxf_TFree(Pext_send_data);
   jxf_TFree(Pext_data);
   jxf_TFree(P_diag_pass);
   jxf_TFree(P_offd_pass);
   jxf_TFree(Pext_pass);
   jxf_TFree(P_diag_start);
   jxf_TFree(P_offd_start);
   jxf_TFree(Pext_start);
   jxf_TFree(Pext_i);
   jxf_TFree(fine_to_coarse);
   jxf_TFree(assigned);
   jxf_TFree(assigned_offd);
   jxf_TFree(pass_pointer);
   jxf_TFree(pass_array);
   jxf_TFree(map_S_to_new);
   jxf_TFree(map_A_to_S);
   jxf_TFree(P_marker);
   if (num_procs > 1) 
   {
   ...
   {
      jxf_PAMGInterpTruncation(par_P, trunc_factor, P_max_elmts);
...
#include "../../include/jxf_smp_forloop.h"
      ...
      permute = jxf_CTAlloc(JXF_Int, new_counter[num_passes-1]);
      ...
      jxf_qsort0(col_map_offd_P, 0, num_cols_offd_P - 1);
      ...
      for (i = 0; i < new_counter[num_passes-1]; i ++)
      {
         ...
         if (k1 != -1)
         {
            permute[i] = jxf_BinarySearch(col_map_offd_P, k1, num_cols_offd_P);
         }
      }
...
#include "../../include/jxf_smp_forloop.h"
   ...
   if (num_procs > 1)
   {
      for (i = 0; i < num_passes - 1; i ++)
      {
         jxf_TFree(new_elmts[i]);
      }
   }
   jxf_TFree(P_marker_offd);
   jxf_TFree(permute);
   jxf_TFree(new_elmts);
   jxf_TFree(new_counter);

   if (num_cols_offd_P)
   {
        jxf_ParCSRMatrixColMapOffd(par_P) = col_map_offd_P;
        jxf_CSRMatrixNumCols(P_offd) = num_cols_offd_P;
   }
