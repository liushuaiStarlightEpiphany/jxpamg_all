...
for sid in 12 22 32 ; do
        mpirun -np 1 ./solver \
        $dataroot/$pb1/csrmat_1023X1023.dat 9104256 272717760 x_${pb}.txt 1e-6 $pb1 $dataroot/$pb1/rhs_1023X1023.dat 0\
        -sid $sid -maxiter 100 -nts $nts -ct 10 -ipt 6  -rlx 3  \
                2>&1 |tee -a ./log/$mydate/$pb-sid$sid-np$np.log
done
UA-AMG
for sid in 12 22 32 ; do
        mpirun -np 1 ./solver \
        $dataroot/$pb1/csrmat_1023X1023.dat 9104256 272717760 x_${pb}.txt 1e-6 $pb1 $dataroot/$pb1/rhs_1023X1023.dat 0\
        -sid $sid -maxiter 100 -nts $nts -ct 66 -ipt 66  -rlx 6 -cyt 2 -str 0.06 -mxct 25 \
                2>&1 |tee -a ./log/$mydate/$pb-sid$sid-np$np.log
done
