...
)
{
    ...
    // 5. 设置GMRES求解器
    JX_GMRESSetup(gmres_solver, (JX_Matrix)A_high, 
                  (JX_Vector)b_high, (JX_Vector)x_high);
    
    // 6. 求解（使用高精度GMRES + 混合精度CPR预条件器）
    if (myid == 0) {
        printf("\nStarting GMRES with mixed precision CPR preconditioner...\n");
    ...
    JX_GMRESGetFinalRelativeResidualNorm(gmres_solver, &final_rel_res_norm);
    
    if (myid == 0) {
        printf("\nGMRES with mixed precision CPR completed in %.6f seconds\n", 
...
int main(int argc, char** argv)
{
    ...
    if (myid == 0) {
        printf("\n=================================================\n");
        printf("Mixed Precision CPR-GMRES Solver\n");
