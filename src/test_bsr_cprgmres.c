...
int main(int argc, char** argv) {
    ...
    if (myid == 0) {
        printf("  running cpr-gmres with %d MPI-processes\n", size);
    }
    ...
    JX_GMRESSetup(gmres_solver, (JX_Matrix)A_parbsr,
    ...
    // ------------------------------------------------------------
    if (myid == 0) {
        printf("\nSolving with CPR-GMRES (k_dim=%d, max_iter=%d, tol=%.1e)...\n", k_dim, max_iter, tol);
        fflush(stdout);
    }
    ...
    JX_GMRESSolve(gmres_solver, (JX_Matrix)A_parbsr,
