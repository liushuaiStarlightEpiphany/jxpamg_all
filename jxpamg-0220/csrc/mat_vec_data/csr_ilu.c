...
static void jx_qsplit(JX_Real* a, JX_Int* ind, JX_Int n, JX_Int ncut);
static void jx_check_col_index(JX_Int row, JX_Int num, JX_Int* q);
static void jx_sortrow(JX_Int num, JX_Int* q);

/**
 * \fn JX_Int jx_SymbFactor(JX_Int n, JX_Int* rwptr, JX_Int* colind, JX_Int levfill,
 *                                           JX_Int nzmax, JX_Int* nzlu,
 *                                           JX_Int* ijlu, JX_Int* uptr)
 *
...
JX_Int jx_SymbFactor(JX_Int n, JX_Int* rwptr, JX_Int* colind, JX_Int levfill, JX_Int nzmax, JX_Int* nzlu, JX_Int* ijlu,
                             JX_Int* uptr)
{
#if DEBUG_MODE > 0
    ...
    JX_Int  icolindj, ijlum, i, j, k, m, ibegin, iend, Ujbeg, Ujend, NE;
    JX_Int  head, prev, lm, actlev, lowct, k1, k2, levp1, lmk, nzi, rowct;
    JX_Int  cinindex = 0;
    JX_Int *rowll, *lastcol, *levels;
    JX_Int  ierr;

    rowll   = (JX_Int*)calloc(n, sizeof(JX_Int));
    lastcol = (JX_Int*)calloc(n, sizeof(JX_Int));
    levels  = (JX_Int*)calloc(nzmax, sizeof(JX_Int));
    ...
    *nzlu = n + 1;
    ...
    for (i = 1; i <= n; ++i) { // 100
        ...
        if (nzi > 1) {
            ...
            //  ---------------------------------------------------------
            jx_sortrow(nzi - 1, &rowll[1]);

            // check col index
            jx_check_col_index(i, nzi - 1, &rowll[1]);
            //  ---------------------------------------------------------
                ...
                if (levfill != 0) {
                    ...
                    for (m = Ujbeg; m <= Ujend; ++m) { // 60
                        ...
                        if (lastcol[ijlum] != i) {
                            ...
                            levels[ijlum] = jx_min(levels[ijlum], actlev);
                        }
...
/**
 * \fn void jx_iluk (JX_Int n, JX_Real *a,JX_Int *ja, JX_Int *ia, JX_Int lfil,
 *                     JX_Real *alu, JX_Int *jlu, JX_Int iwk, JX_Int *ierr, JX_Int *nzlu)
 *
 ...
 * \date   11/03/2024
 */
void jx_iluk(JX_Int n, JX_Real* a, JX_Int* ja, JX_Int* ia, JX_Int lfil, JX_Real* alu, JX_Int* jlu, JX_Int iwk,
                 JX_Int* ierr, JX_Int* nzlu)
{
#if DEBUG_MODE > 0
    printf("### DEBUG: [-Begin-] %s ...\n", __FUNCTION__);
#endif
    ...
    // locals
    JX_Int   ju0, k, j1, j2, j, ii, i, lenl, lenu, jj, jrow, jpos, n2, jlev, NE;
    JX_Real  t, s, fact;
    JX_Int   cinindex = 0;
    JX_Real* w;
    JX_Int * ju, *jw, *levs;
    ...
    w    = (JX_Real*)calloc(n, sizeof(JX_Real));
    ju   = (JX_Int*)calloc(n, sizeof(JX_Int));
    jw   = (JX_Int*)calloc(3 * n, sizeof(JX_Int));
    levs = (JX_Int*)calloc(iwk, sizeof(JX_Int));
        ...
        for (k = ju[jrow]; k <= jlu[jrow + 1] - 1; ++k) { // 203
            ...
            if (j >= ii) {
                ...
                if (jpos == 0) {
                    ...
                    w[jpos]       = w[jpos] - s;
                    jw[n2 + jpos] = jx_min(jw[n2 + jpos], jlev + levs[k] + 1);
                }
            } else {
...
/**
 * \fn void jx_ilut (JX_Int n, JX_Real *a, JX_Int *ja, JX_Int *ia, JX_Int lfil, JX_Real droptol,
 *                     JX_Real *alu, JX_Int *jlu, JX_Int iwk, JX_Int *ierr, JX_Int *nz)
 *
 ...
 * \date   11/03/2024
 */
void jx_ilut(JX_Int n, JX_Real* a, JX_Int* ja, JX_Int* ia, JX_Int lfil, JX_Real droptol, JX_Real* alu, JX_Int* jlu,
                 JX_Int iwk, JX_Int* ierr, JX_Int* nz)
{
#if DEBUG_MODE > 0
    printf("### DEBUG: [-Begin-] %s ...\n", __FUNCTION__);
#endif
    ...
    // locals
    JX_Int   ju0, k, j1, j2, j, ii, i, lenl, lenu, jj, jrow, jpos, NE, len;
    JX_Real  t, s, fact, tmp;
    JX_Int   cinindex = 0;
    JX_Real *w, *tnorm;
    JX_Int * ju, *jw;
    ...
    ju    = (JX_Int*)calloc(n, sizeof(JX_Int));
    jw    = (JX_Int*)calloc(2 * n, sizeof(JX_Int));
    w     = (JX_Real*)calloc(n + 1, sizeof(JX_Real));
    tnorm = (JX_Real*)calloc(n, sizeof(JX_Real));
    ...
    for (ii = 1; ii <= n; ++ii) {
        ...
        j2  = ia[ii + 1] - 1;
        tmp = 0.0;
        for (k = j1; k <= j2; ++k) tmp = tmp + jx_abs(a[k]);
        tmp       = tmp / (JX_Real)(j2 - j1 + 1);
    ...
    F150:
        ...
        if (jx_abs(fact) <= droptol) goto F150;
    ...
    F160:
        ...
        // update L-matrix
        lenl = len;
        len  = jx_min(lenl, lfil);

        // sort by quick-split
        jx_qsplit(&w[1], &jw[1], lenl, len);
        ...
        len = 0;
        for (k = 1; k <= lenu - 1; ++k) {
            //		if ( jx_abs(w[ii + k])  >  droptol*tnorm )
            if (jx_abs(w[ii + k]) > tnorm[ii]) {
                ++len;
                w[ii + len]  = w[ii + k];
                jw[ii + len] = jw[ii + k];
            }
        ...
        lenu = len + 1;
        len  = jx_min(lenu, lfil);

        jx_qsplit(&w[ii + 1], &jw[ii + 1], lenu - 1, len);
...
/**
 * \fn void jx_ilutp (JX_Int n, JX_Real *a, JX_Int *ja, JX_Int *ia, JX_Int lfil, JX_Real droptol,
 *                      JX_Real permtol, JX_Int mbloc, JX_Real *alu, JX_Int *jlu, JX_Int *iperm,
 *                      JX_Int iwk, JX_Int *ierr, JX_Int *nz)
 *
 ...
 * \date   11/03/2024
 */
void jx_ilutp(JX_Int n, JX_Real* a, JX_Int* ja, JX_Int* ia, JX_Int lfil, JX_Real droptol, JX_Real permtol, JX_Int mbloc,
                  JX_Real* alu, JX_Int* jlu, JX_Int* iperm, JX_Int iwk, JX_Int* ierr, JX_Int* nz)
{
#if DEBUG_MODE > 0
    printf("### DEBUG: [-Begin-] %s ...\n", __FUNCTION__);
#endif
    ...
    // local variables
    JX_Int   k, i, j, jrow, ju0, ii, j1, j2, jpos, len, imax, lenu, lenl, jj, icut, NE;
    JX_Real  s, tmp, tnorm, xmax, xmax0, fact, t;
    JX_Int   cinindex = 0;
    JX_Real* w;
    JX_Int * ju, *jw;
    ...
    for (ii = 1; ii <= n; ++ii) { // 500
        ...
        tnorm = 0.0;
        for (k = j1; k <= j2; ++k) tnorm = tnorm + jx_abs(a[k]); // 501
        if (tnorm == 0.0) goto F999;
        tnorm = tnorm / (JX_Real)(j2 - j1 + 1);
...
// JX_Int jx_CSRMatrixSymbFactor(jx_CSRMatrix* A, JX_Int levfill, JX_Int nzmax, JX_Int* nzlu, JX_Int* ijlu, JX_Int* uptr)
// {
//     JX_Int  row = jx_CSRMatrixNumRows(A);
...
JX_Int jx_CSRMatrixILUK(jx_CSRMatrix* A, JX_Int levfill, JX_Real* alu, JX_Int* jlu, JX_Int iwk, JX_Int* nzlu)
{
    JX_Int      ierr   = JX_SUCCESS;
    JX_Int      row    = jx_CSRMatrixNumRows(A);
    JX_Int*     A_i    = jx_CSRMatrixI(A);
    JX_Int*     A_j    = jx_CSRMatrixJ(A);
    JX_Complex* A_data = jx_CSRMatrixData(A);

    jx_iluk(row, A_data, A_j, A_i, levfill, alu, jlu, iwk, &ierr, nzlu);

    return ierr;
}

JX_Int jx_CSRMatrixILUT(jx_CSRMatrix* A, JX_Int levfill, JX_Real droptol, JX_Real* alu, JX_Int* jlu, JX_Int iwk,
                                JX_Int* nzlu)
{
    JX_Int      ierr   = JX_SUCCESS;
    JX_Int      row    = jx_CSRMatrixNumRows(A);
    ...
    jx_ilut(row, A_data, A_j, A_i, levfill, droptol, alu, jlu, iwk, &ierr, nzlu);

    return ierr;
}

JX_Int jx_CSRMatrixILUTP(jx_CSRMatrix* A, JX_Int levfill, JX_Real droptol, JX_Real permtol, JX_Int mbloc, JX_Real* alu,
                                 JX_Int* jlu, JX_Int* iperm, JX_Int iwk, JX_Int* nzlu)
{
    JX_Int      ierr   = JX_SUCCESS;
    JX_Int      row    = jx_CSRMatrixNumRows(A);
    JX_Int*     A_i    = jx_CSRMatrixI(A);
    ...
    jx_ilutp(row, A_data, A_j, A_i, levfill, droptol, permtol, mbloc, alu, jlu, iperm, iwk, &ierr, nzlu);
...
/**
 * \fn static void jx_qsplit (JX_Real *a, JX_Int *ind, JX_Int n, JX_Int ncut)
 *
...
/**
 * \fn static void jx_sortrow (JX_Int num,JX_Int *q)
 *
...
static void jx_sortrow(JX_Int num, JX_Int* q)
{
    ...
    JX_Int key, icn, ih, ii, i, j, jj;
    JX_Int iinc[6] = {0, 1, 4, 13, 40, 121};
...
/**
 * \fn static void jx_check_col_index (JX_Int row, JX_Int num, JX_Int *q)
 *
