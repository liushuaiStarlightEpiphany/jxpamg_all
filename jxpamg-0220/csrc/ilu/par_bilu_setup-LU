...
JX_Int
jx_BILUSetup(void *ilu_vdata, jx_ParBSRMatrix *A, jx_ParVector *f, jx_ParVector *u)
{
    ...
    JX_Int n = jx_BSRMatrixNumRows(jx_ParBSRMatrixDiag(A));
    ...
    jx_ParBSRMatrixDestroy(matL);
    matL = NULL;
    jx_ParBSRMatrixDestroy(matU);
    matU = NULL;
    jx_ParBSRMatrixDestroy(matmL);
    matmL = NULL;
    jx_ParBSRMatrixDestroy(matmU);
    matmU = NULL;
    jx_ParBSRMatrixDestroy(matS);
    matS = NULL;
    ...
    JX_Int vec_global_size = jx_ParBSRMatrixGlobalNumRows(A) * jx_ParBSRMatrixBlockSize(A);
    JX_Int *scalar_part = NULL;
    jx_ParBSRMatrixGetRowPartitioning(A, &scalar_part);
    Utemp = jx_ParVectorCreate(jx_ParBSRMatrixComm(A),
                               vec_global_size,
                               scalar_part);
    jx_ParVectorInitialize(Utemp);
    jx_ParILUDataUTemp(ilu_data) = Utemp;

    Ftemp = jx_ParVectorCreate(jx_ParBSRMatrixComm(A),
                               vec_global_size,
                               scalar_part);
    jx_ParVectorInitialize(Ftemp);
    jx_ParILUDataFTemp(ilu_data) = Ftemp;
    jx_TFree(scalar_part);
    ...
    switch (ilu_type)
    {
        ...
        case 3:
        {
            jx_ILUSetupFGPBILU_v3(matA, sweep, &matL, &D_array, &matU, &matS, &u_end);
        }
        break;
    }
    ...
    {
        MPI_Comm m_comm = jx_ParBSRMatrixComm(matA);
        jx_BSRMatrix *m_diag = jx_ParBSRMatrixDiag(matA);
        JX_Int *m_diag_i = jx_BSRMatrixI(m_diag);
        jx_BSRMatrix *m_offd = jx_ParBSRMatrixOffd(matA);
        JX_Int *m_offd_i = jx_BSRMatrixI(m_offd);
        JX_Int m_nrows = jx_BSRMatrixNumRows(m_diag);
        JX_Real m_local_nnz = (JX_Real)m_diag_i[m_nrows] + (JX_Real)m_offd_i[m_nrows];
        JX_Real m_total_nnz;
        jx_MPI_Allreduce(&m_local_nnz, &m_total_nnz, 1, JX_MPI_REAL, MPI_SUM, m_comm);
        jx_ParBSRMatrixDNumNonzeros(matA) = m_total_nnz;
    }
    nnzS = 0.0;

    /* size_C is the size of global coarse grid, upper left part */
    size_C = jx_ParBSRMatrixGlobalNumRows(matA);
...
JX_Int jx_ILUSetupFGPBILU_v3(jx_ParBSRMatrix *A,
                            JX_Int sweep,
                            jx_ParBSRMatrix **Lptr,
                            jx_ParVector **D,
                            jx_ParBSRMatrix **Uptr,
                            jx_ParBSRMatrix **Sptr,
                            JX_Int **u_end)
{
    MPI_Comm comm = jx_ParBSRMatrixComm(A);
    JX_Int num_procs, my_id;
    ...
    JX_Int first_col_diag_A = jx_ParBSRMatrixFirstColDiag(A);
    JX_Int num_cols_diag_A = jx_BSRMatrixNumCols(A_diag);
    JX_Int last_col_diag_A = first_col_diag_A + num_cols_diag_A - 1;
    JX_Int *col_map_offd_A = jx_ParBSRMatrixColMapOffd(A);
    // JX_Int L_offd_cols =  0;
    // JX_Int U_offd_cols =  0;

    /* Data objects for L, D, U */
    jx_ParBSRMatrix *matL = NULL, *matU = NULL;
    jx_BSRMatrix *L_diag = NULL, *U_diag = NULL;
    JX_Real *D_data = NULL;
    JX_Real *d_data = NULL;
    JX_Real *L_diag_data = NULL, *U_diag_data = NULL;
    JX_Int *L_diag_i = NULL, *L_diag_j = NULL;
    ...
    JX_Int L_offd_cols =  jx_BSRMatrixNumCols(A_offd);
    JX_Int U_offd_cols =  jx_BSRMatrixNumCols(A_offd);

    jx_BSRMatrix *L_offd = NULL, *U_offd = NULL;
    JX_Real *L_offd_data = NULL, *U_offd_data = NULL;
    JX_Int *L_offd_i = NULL, *L_offd_j = NULL;
    JX_Int *U_offd_i = NULL, *U_offd_j = NULL;
    JX_Int LO_nnz = 0, UO_nnz = 0;
    ...
    C_diag_data = jx_TAlloc(JX_Real, nnz_A * bnnz);
    C_offd_data = jx_TAlloc(JX_Real, nnz_AO * bnnz);
...
#pragma omp single
    ...
    D_data = jx_TAlloc(JX_Real, n * bnnz);
    d_data = jx_TAlloc(JX_Real, n * bnnz);
    ...
    L_diag_data = jx_TAlloc(JX_Real, LD_nnz * bnnz);
    U_diag_data = jx_TAlloc(JX_Real, UD_nnz * bnnz);
...
#pragma omp parallel num_threads(nt)
    {
        ...
        for (int r = istart; r < iend && r < n; r++)
        {
            ...
            {
                JX_Int col = A_diag_j[jj];
                JX_Real* A_blk = A_diag_data + jj * bnnz;
                if (col < r)
                {
                    L_diag_j[workL[tid] + cntL] = col;
                    JX_Real* L_blk = L_diag_data + (workL[tid] + cntL) * bnnz;
                    JX_Real* D_inv_col = D_data + col * bnnz;
                    jx_bsr_block_matmul(A_blk, D_inv_col, L_blk, block_size);
                    cntL++;
                }
                else if (col > r)
                {
                    U_diag_j[workU[tid] + cntU] = col;
                    JX_Real* U_blk = U_diag_data + (workU[tid] + cntU) * bnnz;
                    jx_bsr_block_copy(A_blk, U_blk, 1.0, block_size);
                    cntU++;
                }
                else
                {
                    JX_Real* diag_blk = d_data + r * bnnz;
                    jx_bsr_block_copy(A_blk, diag_blk, 1.0, block_size);
                    /* guard diagonal */
                    JX_Real det = jx_bsr_block_det(diag_blk, block_size);
                    if (fabs(det) < MAT_TOL) {
                        memset(diag_blk, 0, bnnz * sizeof(JX_Real));
                        for (int k = 0; k < block_size; k++) diag_blk[k * block_size + k] = 1.0;
                    }
                    JX_Real* D_blk = D_data + r * bnnz;
                    jx_bsr_block_copy(diag_blk, D_blk, 1.0, block_size);
                    jx_bsr_block_inv(D_blk, block_size);
                }
            }
        }
    }
...
#pragma omp single
        ...
        L_offd_j = jx_TAlloc(JX_Int, LO_nnz);
        U_offd_j = jx_TAlloc(JX_Int, UO_nnz);
        L_offd_data = jx_TAlloc(JX_Real, LO_nnz * bnnz);
        U_offd_data = jx_TAlloc(JX_Real, UO_nnz * bnnz);
        // L_offd_data = jx_TAlloc(JX_Real, LO_nnz);
        // U_offd_data = jx_TAlloc(JX_Real, UO_nnz);
...
#pragma omp parallel num_threads(nt)
        {
            ...
            for (int r = istart; r < iend && r < n; r++)
            {
                ...
                for (int jj = A_offd_i[r]; jj < A_offd_i[r + 1]; jj++)
                {
                    JX_Int offd_idx = A_offd_j[jj];
                    JX_Int global_col_idx = col_map_offd_A[offd_idx];
                    JX_Real* A_offd_blk = A_offd_data + jj * bnnz;
                    if (global_col_idx < global_row_idx)
                    {
                        L_offd_j[workLO[tid] + cntL] = offd_idx;
                        JX_Real* L_offd_blk = L_offd_data + (workLO[tid] + cntL) * bnnz;
                        jx_bsr_block_copy(A_offd_blk, L_offd_blk, 1.0, block_size);
                        cntL++;
                    }
                    else
                    {
                        U_offd_j[workUO[tid] + cntU] = offd_idx;
                        JX_Real* U_offd_blk = U_offd_data + (workUO[tid] + cntU) * bnnz;
                        jx_bsr_block_copy(A_offd_blk, U_offd_blk, 1.0, block_size);
                        cntU++;
                    }
                }
            }
...
//     }
    ...
    matL = jx_ParBSRMatrixCreate(comm,
                                 jx_ParBSRMatrixBlockSize(A),
                                 jx_ParBSRMatrixGlobalNumRows(A),
                                 jx_ParBSRMatrixGlobalNumCols(A),
                                 jx_ParBSRMatrixRowStarts(A),
                                 jx_ParBSRMatrixColStarts(A),
                                 L_offd_cols,
                                 LD_nnz,
                                 LO_nnz );
    /* attach diag/offd pointers for L */
    L_diag = jx_ParBSRMatrixDiag(matL);
    jx_BSRMatrixI(L_diag) = L_diag_i;
    jx_BSRMatrixJ(L_diag) = L_diag_j;
    jx_BSRMatrixData(L_diag) = L_diag_data;
    // jx_CSRMatrixNumRows(L_diag); /* just ensure object exists */
    // /* offd */
    // jx_ParCSRMatrixOffd(matL);       /* ensure offd exists if function provided */
    // jx_ParCSRMatrixColMapOffd(matL); /* ensure colmap exists if needed */
    ...
    /* For U */
    matU = jx_ParBSRMatrixCreate(comm,
                                 jx_ParBSRMatrixBlockSize(A),
                                 jx_ParBSRMatrixGlobalNumRows(A),
                                 jx_ParBSRMatrixGlobalNumCols(A),
                                 jx_ParBSRMatrixRowStarts(A),
                                 ...
                                 UD_nnz,
                                 UO_nnz );
    U_diag = jx_ParBSRMatrixDiag(matU);
    jx_BSRMatrixI(U_diag) = U_diag_i;
    jx_BSRMatrixJ(U_diag) = U_diag_j;
    jx_BSRMatrixData(U_diag) = U_diag_data;

    /* D_array stores inverse diagonal blocks: global size = block_rows * bnnz */
    JX_Int global_block_rows = jx_ParBSRMatrixGlobalNumRows(A);
    JX_Int global_vec_size = global_block_rows * bnnz;
    jx_ParVector *D_array = jx_ParVectorCreate(jx_ParBSRMatrixComm(A),
                                               global_vec_size,
                                               jx_ParBSRMatrixRowStarts(A));
    jx_ParVectorInitialize(D_array);
    jx_Vector *D_local = jx_ParVectorLocalVector(D_array);
    ...
    if (num_procs > 1)
    {
        ...
        /* offd 部分指针绑定 */
        jx_BSRMatrix *L_offd = jx_ParBSRMatrixOffd(matL);
        jx_BSRMatrix *U_offd = jx_ParBSRMatrixOffd(matU);

        jx_BSRMatrixI(L_offd) = L_offd_i;
        jx_BSRMatrixJ(L_offd) = L_offd_j;
        jx_BSRMatrixData(L_offd) = L_offd_data;
        jx_BSRMatrixNumCols(L_offd) = L_offd_cols;

        jx_BSRMatrixI(U_offd) = U_offd_i;
        jx_BSRMatrixJ(U_offd) = U_offd_j;
        jx_BSRMatrixData(U_offd) = U_offd_data;
        jx_BSRMatrixNumCols(U_offd) = U_offd_cols;
        ...
        /* 通信包创建 */
        jx_BlockMatvecCommPkgCreate(matL);
        jx_BlockMatvecCommPkgCreate(matU);
    }
...
#pragma omp parallel num_threads(nt)
        {
            ...
            for (int r = istart; r < iend && r < n; r++)
            {
                ...
                {
                    JX_Int col = A_diag_j[jj];
                    JX_Real *C_blk = C_diag_data + jj * bnnz;
                    JX_Real *A_blk = A_diag_data + jj * bnnz;
                    jx_bsr_block_copy(A_blk, C_blk, 1.0, block_size);
                    for (int ljj = L_diag_i[r]; ljj < L_diag_i[r + 1]; ljj++)
                    {
                        JX_Int k = L_diag_j[ljj];
                        for (int ujj = U_diag_i[k]; ujj < U_diag_i[k + 1]; ujj++)
                        {
                            if (U_diag_j[ujj] == col)
                            {
                                JX_Real *L_blk = L_diag_data + ljj * bnnz;
                                JX_Real *U_blk = U_diag_data + ujj * bnnz;
                                JX_Real temp[25];
                                jx_bsr_block_matmul(L_blk, U_blk, temp, block_size);
...
#pragma omp parallel num_threads(nt)
        {
            ...
            for (int r = istart; r < iend && r < n; r++)
            {
                ...
                if (diag_jj >= 0)
                {
                    JX_Real *C_blk = C_diag_data + diag_jj * bnnz;
                    JX_Real *diag_blk = d_data + r * bnnz;
                    jx_bsr_block_copy(C_blk, diag_blk, 1.0, block_size);
                    JX_Real det = jx_bsr_block_det(diag_blk, block_size);
                    if (fabs(det) < MAT_TOL)
                    {
                        for (int b = 0; b < bnnz; b++)
                        ...
                        for (int kk = 0; kk < block_size; kk++)
                            diag_blk[kk * block_size + kk] = (JX_Real)1.0;
                    }
                    JX_Real *D_blk = D_data + r * bnnz;
                    jx_bsr_block_copy(diag_blk, D_blk, 1.0, block_size);
                    jx_bsr_block_inv(D_blk, block_size);
                ...
                for (int jj = A_diag_i[r]; jj < A_diag_i[r + 1]; jj++)
                {
                    ...
                    if (col < r)
                    {
                        JX_Real *L_blk = L_diag_data + localLpos * bnnz;
                        JX_Real *D_col = D_data + col * bnnz;
                        jx_bsr_block_matmul(C_blk, D_col, L_blk, block_size);
                        localLpos++;
                    }
                    else if (col > r)
                    {
                        JX_Real *U_blk = U_diag_data + localUpos * bnnz;
                        jx_bsr_block_copy(C_blk, U_blk, 1.0, block_size);
                        localUpos++;
                    }
                }
            }
...
#pragma omp parallel num_threads(nt)
            {
                ...
                for (int r = istart; r < iend && r < n; r++)
                {
                    for (int jj = A_offd_i[r]; jj < A_offd_i[r + 1]; jj++)
                    {
                        JX_Real *C_blk = C_offd_data + jj * bnnz;
                        JX_Real *A_blk = A_offd_data + jj * bnnz;
                        jx_bsr_block_copy(A_blk, C_blk, 1.0, block_size);
                    }
                    ...
                    for (int jj = A_offd_i[r]; jj < A_offd_i[r + 1]; jj++)
                    {
                        ...
                        if (gcol > global_row_idx)
                        {
                            JX_Real *U_blk = U_offd_data + baseU * bnnz;
                            jx_bsr_block_copy(C_blk, U_blk, 1.0, block_size);
                            baseU++;
                        }
                        else if (gcol < global_row_idx)
                        {
                            JX_Real *L_blk = L_offd_data + baseL * bnnz;
                            jx_bsr_block_copy(C_blk, L_blk, 1.0, block_size);
                            baseL++;
                        }
                    }
                }
            }
            /* Note: the original CSR version called jx_ParCSRMatrixM(matL, D_array)
    ...
    local_nnz = (JX_Real)(UD_nnz + UO_nnz);
    jx_MPI_Allreduce(&local_nnz, &total_nnz, 1, JX_MPI_REAL, MPI_SUM, comm);
    jx_ParBSRMatrixDNumNonzeros(matU) = total_nnz;

    local_nnz = (JX_Real)(LD_nnz + LO_nnz);
    jx_MPI_Allreduce(&local_nnz, &total_nnz, 1, JX_MPI_REAL, MPI_SUM, comm);
    jx_ParBSRMatrixDNumNonzeros(matL) = total_nnz;
...
void jx_ParBSRMatrixMatmul_Apattern(jx_ParBSRMatrix *A, jx_ParBSRMatrix *L, jx_ParBSRMatrix *B,
                                    JX_Real *C_diag_data, JX_Real *C_offd_data)
{
    ...
    jx_BSRMatrix *B_diag = jx_ParBSRMatrixDiag(B);
    jx_BSRMatrix *B_offd = jx_ParBSRMatrixOffd(B);
    JX_Int *B_diag_i = jx_BSRMatrixI(B_diag);
    JX_Int *B_diag_j = jx_BSRMatrixJ(B_diag);
    JX_Real *B_diag_data = jx_CSRMatrixData(B_diag);
    JX_Int *B_offd_i = jx_BSRMatrixI(B_offd);
    JX_Int *B_offd_j = jx_BSRMatrixJ(B_offd);
    JX_Real *B_offd_data = jx_BSRMatrixData(B_offd);

    jx_BSRMatrix *L_diag = jx_ParBSRMatrixDiag(L);
    jx_BSRMatrix *L_offd = jx_ParBSRMatrixOffd(L);
    JX_Int *L_diag_i = jx_BSRMatrixI(L_diag);
    JX_Int *L_diag_j = jx_BSRMatrixJ(L_diag);
    JX_Real *L_diag_data = jx_BSRMatrixData(L_diag);
    JX_Int *L_offd_i = jx_BSRMatrixI(L_offd);
    JX_Int *L_offd_j = jx_BSRMatrixJ(L_offd);
    JX_Real *L_offd_data = jx_BSRMatrixData(L_offd);

    /* local sizes / meta */
    JX_Int num_rows_diag_A = jx_BSRMatrixNumRows(A_diag);
    JX_Int num_cols_diag_A = jx_BSRMatrixNumCols(A_diag);
    JX_Int num_cols_offd_A = jx_BSRMatrixNumCols(A_offd);
    JX_Int first_col_diag_A = jx_ParBSRMatrixFirstColDiag(A);
    JX_Int last_col_diag_A = first_col_diag_A + num_cols_diag_A - 1;

    JX_Int first_col_diag_B = jx_ParBSRMatrixFirstColDiag(B);
    JX_Int num_cols_diag_B = jx_BSRMatrixNumCols(B_diag);
    JX_Int last_col_diag_B = first_col_diag_B + num_cols_diag_B - 1;

    jx_BSRMatrix *Bs_ext = NULL;
    JX_Real *Bs_ext_data = NULL;
    JX_Int *Bs_ext_i = NULL;
    JX_Int *Bs_ext_j = NULL;
    JX_Int *B_ext_diag_i = NULL, *B_ext_offd_i = NULL;
    ...
    if (num_procs > 1)
    {
         ...
         * If there exists no CommPkg for A, a CommPkg is generated using
         * equally load balanced partitionings within
         * jx_ParBSRMatrixExtractBExt
         *--------------------------------------------------------------------*/
        Bs_ext = jx_ParBSRMatrixExtractBExt(B, A, 1);
        Bs_ext_data = jx_BSRMatrixData(Bs_ext);
        Bs_ext_i = jx_BSRMatrixI(Bs_ext);
        Bs_ext_j = jx_BSRMatrixJ(Bs_ext);
    }
    B_ext_diag_i = jx_CTAlloc(JX_Int, num_cols_offd_A + 1);
    B_ext_offd_i = jx_CTAlloc(JX_Int, num_cols_offd_A + 1);
    B_ext_diag_size = 0;
    ...
    #endif
    {
        ...
        for (i = ns; i < ne; i++)
        {
            for (j = Bs_ext_i[i]; j < Bs_ext_i[i + 1]; j++)
                ...
                {
                    B_ext_offd_j[cnt_offd] = Bs_ext_j[j];
                    JX_Real* src_blk = Bs_ext_data + j * bnnz;
                    JX_Real* dst_blk = B_ext_offd_data + cnt_offd * bnnz;
                ...
                else
                {
                    B_ext_diag_j[cnt_diag] = Bs_ext_j[j] - first_col_diag_B;
                    JX_Real* src_blk = Bs_ext_data + j * bnnz;
                    JX_Real* dst_blk = B_ext_diag_data + cnt_diag * bnnz;
                    for (int k = 0; k < bnnz; k++) dst_blk[k] = src_blk[k];
                    cnt_diag++;
                }
        }
    ...
    if (num_cols_offd_A)
    {
        ...
        for (ii1 = 0; ii1 < num_rows_diag_A; ii1++)
        {
            for (jj1 = A_offd_i[ii1]; jj1 < A_offd_i[ii1 + 1]; jj1++)
            {
                jx_bsr_block_copy(A_offd_data + jj1 * bnnz, C_offd_data + jj1 * bnnz, 1.0, block_size);
                target_col = A_offd_j[jj1];
                global_col = col_map_offd_A[target_col];
                ...
                for (jj2 = L_offd_i[ii1]; jj2 < L_offd_i[ii1 + 1]; jj2++)
                {
                    ...
                    for (jj3 = B_ext_offd_i[i2]; jj3 < B_ext_offd_i[i2 + 1]; jj3++)
                    {
                        if (B_ext_offd_j[jj3] == global_col)
                        {
                            jx_bsr_block_matmul_add(L_offd_data + jj2 * bnnz,
                ...
                for (jj2 = L_diag_i[ii1]; jj2 < L_diag_i[ii1 + 1]; jj2++)
                {
                    ...
                    for (jj3 = B_offd_i[i2]; jj3 < B_offd_i[i2 + 1]; jj3++)
                    {
                        if (B_offd_j[jj3] == target_col)
                        {
                            jx_bsr_block_matmul_add(L_diag_data + jj2 * bnnz,
    ...
    for (ii1 = 0; ii1 < num_rows_diag_A; ii1++)
    {
        ...
        for (jj1 = A_diag_i[ii1]; jj1 < A_diag_i[ii1 + 1]; jj1++)
        {
            jx_bsr_block_copy(A_diag_data + jj1 * bnnz,
...
JX_Int
jx_ParBSRMatrixM(  jx_ParBSRMatrix *A,jx_ParVector    *x ) //修改是否正确，需要进行确认
{
    jx_ParCSRCommHandle **comm_handle;
    jx_ParCSRCommPkg	*comm_pkg = jx_ParBSRMatrixCommPkg(A);
    jx_BSRMatrix         *diag     = jx_ParBSRMatrixDiag(A);
    JX_Int *diag_i = jx_BSRMatrixI(diag);
    JX_Int *diag_j = jx_BSRMatrixJ(diag);
    JX_Real *diag_data = jx_BSRMatrixData(diag);
    JX_Int n = jx_BSRMatrixNumRows(diag);
    JX_Int nt = jx_NumThreads();
    JX_Int ROW_OMP;
        
    jx_BSRMatrix         *offd     = jx_ParBSRMatrixOffd(A);
    JX_Int *offd_i = jx_BSRMatrixI(offd);
    JX_Int *offd_j = jx_BSRMatrixJ(offd);
    JX_Real *offd_data = jx_BSRMatrixData(offd);

    jx_Vector            *x_local  = jx_ParVectorLocalVector(x);   
    JX_Int         num_rows = jx_ParBSRMatrixGlobalNumRows(A);
    JX_Int         num_cols = jx_ParBSRMatrixGlobalNumCols(A);
    ...
    *--------------------------------------------------------------------*/
    ...
    JX_Int vec_per_offd_block = jx_ParBSRMatrixBlockSize(A);
    if ( num_vectors == 1 )
    {
        x_tmp = jx_SeqVectorCreate( num_cols_offd * vec_per_offd_block );
    }
    else
    {
        jx_assert( num_vectors > 1 );
        x_tmp = jx_SeqMultiVectorCreate( num_cols_offd * vec_per_offd_block, num_vectors );
    }
    ...
    *--------------------------------------------------------------------*/
    ...
    {
        x_buf_data[jv] = jx_CTAlloc( JX_Real, jx_ParCSRCommPkgSendMapStart(comm_pkg, num_sends) * vec_per_offd_block );
    }
    ...
    if ( num_vectors == 1 )
    {
        ...
        for (i = 0; i < num_sends; i ++)
        {
            ...
            {
                JX_Int offd_col = jx_ParCSRCommPkgSendMapElmt(comm_pkg,j);
                JX_Real* src = x_local_data + offd_col * vec_per_offd_block;
    ...
    else
    {
        for (jv = 0; jv < num_vectors; ++ jv)
        {
            ...
            for (i = 0; i < num_sends; i ++)
            {
                ...
                {
                    JX_Int offd_col = jx_ParCSRCommPkgSendMapElmt(comm_pkg,j);
                    JX_Real* src = x_local_data + jv*vecstride + idxstride*offd_col * vec_per_offd_block;
                    for (int k = 0; k < vec_per_offd_block; k++)
   ...
   /* >>> ... The assert is because the following loop only works for 'column' storage of a multivector <<<
    ...
    {
        comm_handle[jv]
        = jx_ParCSRCommHandleCreate( 1, comm_pkg, x_buf_data[jv], &(x_tmp_data[jv*num_cols_offd*vec_per_offd_block]) );
    }
    ...
    #pragma omp parallel num_threads(nt)
        {
            ...
            for (int r = istart; r < iend && r < n; r++)
            {
                for (int jj = offd_i[r]; jj < offd_i[r+1]; jj++)
                {
                    ...
                    JX_Real* x_blk = x_tmp_data + col * vec_per_offd_block;
                    JX_Real* o_blk = offd_data + jj * vec_per_offd_block;
                    for (int k = 0; k < vec_per_offd_block; k++)
