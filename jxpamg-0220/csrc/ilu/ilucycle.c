                             ...
                             jx_ParVector    *par_x )
{
    ...
    if (num_procs == 1)
    {
        ...
        JX_Int n = jx_VectorSize(f);
        JX_Int k = ilu_data->max_iter;
        JX_Real *r_data = jx_VectorData(ilu_data->res_vec);
        JX_Real *d_data = jx_VectorData(ilu_data->tmp_vec);
        ...
        for (i = 0; i < k; i++)
        {
            ...
            if (ilu_data->drop_tol == 0.0)
                jx_ILUZeroFactorDataCycleA(ilu_data, A, ilu_data->res_vec, ilu_data->tmp_vec);
            else
                jx_ILUZeroFactorDataCycleB(ilu_data, A, ilu_data->res_vec, ilu_data->tmp_vec);
            ...
            if (i < k - 1)
            {
                for (j = 0; j < n; j++)
                    r_data[j] = f_data[j];
                jx_CSRMatrixMatvec(-1.0, A, u, 1.0, ilu_data->res_vec);
            }
        }
    }
    else
        ...
        if (any_has_matA)
        {
            ...
            if (my_id == 0)
            {
                ...
                else
                {
                    ...
                    jx_Vector *z = jx_SeqVectorCreate(n);
                    jx_SeqVectorInitialize(z);
                    ...
                    for (i = 0; i < k; i++)
                    {
                        ...
                        if (i < k - 1)
                        {
                            for (j = 0; j < n; j++)
                                r_data[j] = f_data[j];
                            jx_CSRMatrixMatvec(-1.0, A, z, 1.0, ilu_data->res_vec);
                        }
                    }
