...
//    jx_CSRMatrix      *matALU_d           = jx_ParILUDataMatAILUDevice(ilu_data);
//    jx_CSRMatrix      *matBLU_d           = jx_ParILUDataMatBILUDevice(ilu_data);
//    jx_CSRMatrix      *matE_d             = jx_ParILUDataMatEDevice(ilu_data);
//    jx_CSRMatrix      *matF_d             = jx_ParILUDataMatFDevice(ilu_data);
//    jx_ParCSRMatrix   *Aperm              = jx_ParILUDataAperm(ilu_data);
//    jx_Vector         *Adiag_diag         = jx_ParILUDataADiagDiag(ilu_data);
//    jx_Vector         *Sdiag_diag         = jx_ParILUDataSDiagDiag(ilu_data);
//    jx_ParVector      *Ztemp              = jx_ParILUDataZTemp(ilu_data);
//    JX_Int             test_opt           = jx_ParILUDataTestOption(ilu_data);
// #endif
...
//    JX_Solver          schur_precond      = jx_ParILUDataSchurPrecond(ilu_data);
          ...
          (iter < max_iter))
   {
      switch (tri_solve) 
      {
         ...
         case 14:  //完全分布式 + IR (use BSR version)
            starttime = jx_MPI_Wtime();
            jx_par_Block_Jacobi_TriangularSolve_B((jx_ParBSRMatrix*)matA, F_array, U_array, n, (jx_ParBSRMatrix*)matL, D_array,
                                (jx_ParBSRMatrix*)matU, Utemp, Ftemp, Xtemp, Ytemp, IR_iters, lower_jacobi_iters, upper_jacobi_iters);
            endtime = jx_MPI_Wtime();
            Tri_Total_time +=  endtime - starttime;                                           
         break;
                     ...
                     JX_Int           upper_jacobi_iters)
{
   ...
   // char FileNameCoaMat[256];
   //  jx_sprintf(FileNameCoaMat, "A_CSR_%d", 4);
   //  jx_ParCSRMatrixPrint(A, FileNameCoaMat);
   //  jx_sprintf(FileNameCoaMat, "L_CSR_%d", 4);
   //  jx_ParCSRMatrixPrint(L, FileNameCoaMat);
   //  jx_sprintf(FileNameCoaMat, "U_CSR_%d", 4);
   //  jx_ParCSRMatrixPrint(U, FileNameCoaMat);

   // jx_sprintf(FileNameCoaMat, "  D_CSR_%d", 4);
   //  jx_ParVectorPrint(D_vector, FileNameCoaMat);

   jx_ParVectorCopy(f, ftemp);
   jx_ParCSRMatrixMatvec(alpha, A, u, beta, ftemp);

   // jx_ParVector *ytemp = jx_ParVectorCreate(comm, jx_ParVectorGlobalSize(u), jx_ParVectorPartitioning(u));
   // jx_ParVectorInitialize(ytemp);
   // jx_ParVectorSetPartitioningOwner(ytemp,0);


   // jx_ParVector *unew = jx_ParVectorCreate(comm, jx_ParVectorGlobalSize(u), jx_ParVectorPartitioning(u));
   // jx_ParVectorInitialize(unew);
   // jx_ParVectorSetPartitioningOwner(unew,0);
   // jx_ParVectorSetConstantValues(unew, 0);
